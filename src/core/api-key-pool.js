...
export class APIKeyPool {
    ...
    async getAllKeys() {
        const { [CONSTANTS.SK.AI_KEYS]: allKeys = {} } =
            await storage.local.get(CONSTANTS.SK.AI_KEYS);
    ...
    async getStrategy() {
        const { [CONSTANTS.SK.SETTINGS]: settings = {} } =
            await storage.local.get(CONSTANTS.SK.SETTINGS);
    ...
    async getNextKey() {
        ...
        if (strategy === "random") {
            ...
            dbg.log(
                `APIKeyPool(${this.providerId}) selecting random key idx=${idx}`
            );
        ...
        if (strategy === "sticky-first") {
            dbg.log(
                `APIKeyPool(${this.providerId}) selecting sticky-first key`
            );
            return available[0];
        }
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (Date.now() > cooldownUntil) {
                ...
                dbg.log(
                    `APIKeyPool(${this.providerId}) selected key index=${index}`
                );
                return key;
            }
        }
