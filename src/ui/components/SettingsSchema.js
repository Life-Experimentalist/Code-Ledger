...
function MigrationPanel() {
    ...
    const doMigrate = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setStatus(
                    `Migration complete: ${res.migrated} files updated, ${res.deleted} old files removed.`
                );
            } else {
    ...
    const doReset = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setStatus(
                    `Rebuild complete: ${res.committed} files committed, ${res.deleted} stray files removed.`
                );
            } else {
    ...
    return html`
        <div class="p-6 bg-[#0a0a0f] rounded-2xl border border-white/5">
            <h3
                class="text-sm font-bold text-white uppercase tracking-widest mb-1"
            >
                Repo Migration
            </h3>
            <p class="text-[11px] text-slate-500 mb-4">
                Migrate your GitHub repo to the current file layout, or do a
            ...
            <div class="flex flex-wrap gap-3 items-center">
                ...
                >
                    ${busy ? "Working…" : "Migrate to v2 layout"}
                </button>
                <button
                    onClick=${doReset}
                    disabled=${busy}
...
function BackupRestorePanel() {
    ...
    return html`
        <div class="p-6 bg-[#0a0a0f] rounded-2xl border border-white/5">
            ...
            <p class="text-[11px] text-slate-500 mb-4">
                Export all solved problems and settings to a JSON file, or
            ...
            <div class="flex flex-wrap gap-3 items-center">
                ...
                <label
                    class="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 text-xs rounded-lg cursor-pointer transition-colors ${busy
    ...
    return html`
        ...
        >
            <p class="text-[11px] text-slate-400 mb-3">
                Import your last 20 accepted submissions from LeetCode's public
                API. For full history, visit your profile page on LeetCode and
                use the in-page import button.
            </p>
            <div class="flex items-center gap-3 flex-wrap">
                ${username
                    ? html`<span class="text-xs text-slate-400"
                          >Username:
                          <strong class="text-white">${username}</strong></span
                      >`
                ...
                >
                    ${busy ? "Importing…" : "Import recent solves"}
                </button>
            </div>
            ${status
                ? html`<p
...
function MaintenancePanel({ problems, onRefreshMissing }) {
    ...
    const missingMetadataCount = (problems || []).filter(
    ...
    return html`
        <div
            class="mt-4 p-4 bg-blue-950/20 rounded-xl border border-blue-500/15"
        >
            <h3
                class="text-sm font-bold text-white uppercase tracking-widest mb-1"
            >
            ...
            <p class="text-[11px] text-slate-500 mb-4">
                Refresh missing problem metadata (tags, difficulty, description)
                in the background, one problem at a time.
            </p>
            <div class="flex items-center gap-3 flex-wrap">
                <button
...
export function SettingsSchema({ schema, values, onChange, onSetupRepo }) {
    ...
    useEffect(() => {
        const selected = [values?.aiProvider, values?.aiSecondary].filter(
            Boolean
        );
    ...
    const handleSaveAllKeys = async (providerId, keyField, rawVal) => {
        ...
        try {
            ...
            setTestResults((s) => ({
                ...s,
                [`${keyField}:all`]: e.message || "Failed to save keys",
            }));
        }
    };
    ...
    const handleTestEndpoint = async (providerId, endpointVal, fieldKey) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setTestResults((s) => ({
                    ...s,
                    [fieldKey]: res.error || "Failed",
                }));
            }
        } catch (e) {
            setTestResults((s) => ({
                ...s,
                [fieldKey]: e.message || "Failed",
            }));
        } finally {
    ...
    const doResyncAll = useCallback(async (provider, mode) => {
        ...
        try {
            ...
            setRepoSyncStatus((s) => ({
                ...s,
                [provider]: `Sync failed: ${e.message}`,
            }));
        } finally {
    ...
    const getSyncPreview = useCallback(async () => {
        if (typeof chrome === "undefined" || !chrome.runtime?.id) {
            throw new Error(
                "Sync preview is only available inside the extension."
            );
        }
    ...
    const applySyncImport = useCallback(async (resolvedProblems) => {
        if (typeof chrome === "undefined" || !chrome.runtime?.id) {
            throw new Error(
                "Sync import is only available inside the extension."
            );
        }
        return new Promise((resolve, reject) => {
            chrome.runtime.sendMessage(
    ...
    const shouldRenderField = (section, field) => {
        ...
        if (activeTab === "general") {
            ...
            return !["aiProvider", "aiSecondary", "aiModel"].includes(
                field.key
            );
        }
        return true;
                                        ...
                                        ${(() => {
                                            ...
                                            const repoUrl =
                                                savedRepo && owner
                                                    ...
                                                    : null;
                                            const isSyncing =
                                                repoSyncing[f.provider];
                                            const syncMsg =
                                                repoSyncStatus[f.provider];
                                            const isExtension =
                                                typeof chrome !== "undefined" &&
                                                !!chrome.runtime?.id;
                                            ...
                                            return html`
                                                <div
                                                    class="flex flex-col gap-2 w-full mt-2 p-3 bg-cyan-950/30 border border-cyan-500/20 rounded-lg"
                                                >
                                                    <p
                                                        class="text-[11px] text-cyan-300 font-medium"
                                                    >
                                                        Repository not
                                                        configured
                                                    </p>
        ...
        return html`
            ...
            >
                ...
                !values?.["github_repo"]
                    ? html`
                          ...
                          >
                              ...
                              <div class="flex-1">
                                  <strong>Setup incomplete</strong> — GitHub is
                                  connected but no repository is linked.
                              </div>
                              <button
                                  onClick=${() => onSetupRepo?.()}
                ...
                values?.["github_repo"]
                    ? html`
                          <div
                              class="flex items-center gap-2 text-[11px] text-emerald-400"
                          >
                              <span>✓</span>
                              <span
                                  >Connected & repository linked — syncing is
                                  active.</span
                              >
                          </div>
                      `
                ...
                ${isGitProvider && section.id !== "github"
                    ? html`
                          ...
                          >
                              ⚠️
                              ${CONSTANTS.GIT_PROVIDERS[section.id]?.name ||
                              section.id}
                              support is in testing — do not use in production
                              yet.
                          </div>
                      `
                    : ""}
                ...
                <div class="space-y-4">
                    ...
                    >
                        <div class="flex flex-col gap-1 w-2/3 pr-4">
                            ...
                            <span
                                class="text-[10px] text-slate-500 leading-tight"
                                >Automatically analyze code using AI upon
                                completion.</span
                            >
                        </div>
                    ...
                    >
                        <div class="flex flex-col gap-1 w-2/3 pr-4">
                            ...
                            <span
                                class="text-[10px] text-slate-500 leading-tight"
                                >Preferred AI provider to use for automated
                                reviews.</span
                            >
                        </div>
                        <div class="w-1/3 flex flex-col items-end gap-2">
                            <div class="flex flex-col w-full gap-2">
                                <select
                                    ...
                                    onChange=${(e) =>
                                        onChange("aiProvider", e.target.value)}
                                >
                                    <option value="">None</option>
                                    ${selectableProviders.map(
                                        (pid) =>
                                            html`<option value=${pid}>
                                                ${CONSTANTS.AI_PROVIDERS[pid]
                                                    .name}
                                            </option>`
                                    )}
                                </select>
                                ...
                                ${primaryProvider
                                    ? html`<${ModelSelector}
                                          ...
                                          apiKey=${values[
                                              `${primaryProvider}_keys`
                                          ] || ""}
                                          selectedModel=${values.aiPrimaryModel ||
                                          ""}
                                          onSelect=${(v) =>
                                              onChange("aiPrimaryModel", v)}
                                          endpoint=${values[
                                              `${primaryProvider}_endpoint`
                                          ] || ""}
                    ...
                    >
                        <div class="flex flex-col gap-1 w-2/3 pr-4">
                            ...
                            <span
                                class="text-[10px] text-slate-500 leading-tight"
                                >Fallback provider to be used if the primary
                                fails.</span
                            >
                            <span
                                class="text-[10px] text-cyan-500/80 leading-tight"
                                >You can choose the same provider as primary
                                when the fallback model is different.</span
                            >
                        </div>
                        <div class="w-1/3 flex flex-col items-end gap-2">
                            <div class="flex flex-col w-full gap-2">
                                <select
                                    ...
                                    onChange=${(e) =>
                                        onChange("aiSecondary", e.target.value)}
                                >
                                    <option value="">None</option>
                                    ${selectableProviders.map(
                                        (pid) =>
                                ...
                                ${secondaryProvider
                                    ? html`<${ModelSelector}
                                          ...
                                          apiKey=${values[
                                              `${secondaryProvider}_keys`
                                          ] || ""}
                                          selectedModel=${values.aiSecondaryModel ||
                                          ""}
                                          onSelect=${(v) =>
                                              onChange("aiSecondaryModel", v)}
                                          endpoint=${values[
                                              `${secondaryProvider}_endpoint`
                                          ] || ""}
                                          providerEnabled=${isProviderEffectivelyEnabled(
                                              secondaryProvider
                                          )}
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-500 mt-1">
                                        Provider configuration and key
                                        management.
                                    </p>
                                </div>
                                <label
                                          ...
                                          <div class="space-y-2">
                                              ${keyList.map(
                                                  (k, idx) => html`
                                                      ...
                                                      >
                                                          <span
                                                              class="text-xs text-slate-300 font-mono"
                                                              >${maskKey(
                                                                  k
                                                              )}</span
                                                          >
    ...
    const renderPromptsTab = () => {
        ...
        return html`
            ...
            >
                ...
                >
                    Warning: changing prompts can reduce review quality. Restore
                              ...
                              <div class="flex flex-col gap-2">
                                  ...
                                  >
                                      <div class="font-semibold mb-0.5">
                                          Single commit
                                          <span
                                              class="text-cyan-500/60 font-normal"
                                              >(recommended)</span
                                          >
                                      </div>
                                      <div class="text-[10px] text-slate-500">
                                          All problems in one atomic commit —
                                  ...
                                  >
                                      ...
                                      <div class="text-[10px] text-slate-500">
                                          One backdated commit per problem —
                                          slower, may hit rate limits.
                                      </div>
                                  </button>
                              </div>
                              <button
                                  onClick=${() => setSyncConfirm(null)}
                                  class="text-[11px] text-slate-600 hover:text-slate-400 self-end"
                              >
                                  Cancel
                              </button>
                          </div>
