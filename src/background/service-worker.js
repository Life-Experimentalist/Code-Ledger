...
import { migrateRepo, resetRepo, forceRebuildRepo, detectRepoLayoutVersion, migrateProblemIds } from "./migration-manager.js";
import { SyncEngine, importFromRepo, applyImport } from "./sync-engine.js";
import { detectDuplicate } from "../core/duplicate-detector.js";
import { autoSyncSettings, syncSettingsToGitHub, syncSettingsFromGitHub } from "../core/settings-sync.js";
import {
...
import { needsSettingsCommit, getConfigFileForCommit, clearSettingsCommitFlag, forceCommitSettingsNow } from "../core/settings-auto-commit.js";
...
async function init() {
  ...
  if (!_syncAlarmBound) {
    ...
    try {
      chrome.alarms.create(CONSTANTS.ALARM_NAMES.SYNC, { periodInMinutes: CONSTANTS.SYNC_ALARM_PERIOD_MIN || 30 });
      ...
      chrome.alarms.onAlarm.addListener((alarm) => {
        if (alarm?.name === CONSTANTS.ALARM_NAMES.SYNC) {
          ...
          processAIReviewQueue().catch((e) => dbg.warn("AI review queue processing failed:", e.message));
        }
      });
    } catch (e) {
  ...
  // Initialize AI review queue store
  await initializeReviewQueueStore().catch((e) => dbg.warn("failed to initialize review queue:", e));
...
async function applyFirstRunDefaults() {
  try {
    ...
    await Storage.setSettings({ ...settings, ...updates });
    dbg.log(`applyFirstRunDefaults(): ✓ disabled ${aiProvidersDisabled} AI provider(s), applied defaults`);
  } catch (e) {
...
function getProblemCommitKey(problem = {}) {
  ...
  const id = String(
    problem.id || CONSTANTS.makeProblemId(problem.platform || "unknown", problem.titleSlug || problem.slug || "unknown")
  ).trim();
  if (!id) return "";
...
function _stableJSON(value) {
  ...
  if (Array.isArray(value)) return JSON.stringify(value.map((v) => (typeof v === "string" ? v.trim() : v)));
...
function _isProblemDrifted(local, remote) {
  ...
  const keys = ["title", "difficulty", "code", "tags", "lang", "aiReview", "canonical", "files"];
  ...
  for (let idx = 0; idx < providers.length; idx++) {
    ...
    try {
      ...
      const review = await Promise.race([
        ai.review(problem.code, { ...problem, aiModelOverride: provider.model || "" }),
        new Promise((_, rej) => setTimeout(() => rej(new Error("AI provider timeout")), TIMEOUT_MS)),
      ]);
...
async function commitUpdatedProblem(problem, settings) {
  ...
  try {
    await _commitWithFailover(
      ...
      repoName,
      { date: new Date(problem.timestamp ? (problem.timestamp > 1e10 ? problem.timestamp : problem.timestamp * 1000) : Date.now()) },
      currentSettings,
    );
...
function _normalizeGitTarget(target) {
  ...
  return {
    ...
    owner: target.owner || "",
    repo: String(target.repo || "").replace(/\s+/g, "-").trim(),
  };
}
...
function _getOrderedTargets(settings = {}) {
  ...
  const mirrors = Array.isArray(settings.git_mirrors) ? settings.git_mirrors.map(_normalizeGitTarget).filter(Boolean) : [];
...
async function _commitWithFailover(files, message, repoName, commitOpts, settings) {
  ...
  if (!targets.length) {
    ...
    dbg.log(`_commitWithFailover(): no ordered targets; committing to configured provider ${settings.gitProvider || 'github'} repo ${repoName || settings.github_repo || settings.gitRepo}`);
    await git.commit(files, message, repoName || settings.github_repo || settings.gitRepo, commitOpts);
  ...
  for (const target of targets) {
    ...
    dbg.log(`_commitWithFailover(): attempting target ${target.provider}/${target.owner || ''}/${target.repo}`);
    if (!handler) continue;
    try {
      ...
      if (currentKey !== wonKey) {
        await Storage.setSettings({ ...settings, git_active_primary: target }).catch(() => { });
      }
      dbg.log(`_commitWithFailover(): ✓ succeeded to ${target.provider}/${target.owner || ''}/${target.repo}`);
      return { handler, target };
    } catch (e) {
      lastErr = e;
      dbg.warn(`_commitWithFailover(): ✗ target ${target.provider}/${target.owner ? target.owner + "/" : ""}${target.repo} failed:`, e.message);
    }
  }
...
async function _resolveGitHubContext(settings = null) {
  ...
  const target = (_getOrderedTargets(s).find((t) => t.provider === "github") || _getDefaultPrimaryTarget(s));
...
async function handleSolved(data) {
    ...
    try {
      ...
      try {
        ...
        if (dupResult.isDuplicate) {
          ...
          dbg.log(`handleSolved(): duplicate detected: ${data.titleSlug} matches ${dupResult.duplicateOf}`);
          await Storage.saveProblem(data);
        }
      } catch (dupErr) {
  ...
  try {
    const { findDuplicatesForProblem } = await import("../core/ai-deduplication.js");
    ...
    if (existingProblem && existingProblem.solutions && Array.isArray(existingProblem.solutions)) {
      ...
      if (sameLang.length > 0) {
        const groups = await findDuplicatesForProblem(
          { ...existingProblem, solutions: sameLang.concat([{ code: data.code, lang: data.lang?.name }]) },
        ...
        if (groups.length > 0 && groups[0].length > 1) {
          ...
          dbg.log(`handleSolved(): queued for review (dedup): ${data.titleSlug} (${data.lang?.name})`);
        }
      }
    }
  } catch (err) {
  ...
  dbg.log(`handleSolved(): git config - enabled=${gitEnabled}, skipCommit=${!!data.skipCommit}, forceCommit=${!!data.forceCommit}`);
  if (data.skipCommit) {
    dbg.log(`handleSolved(): skipCommit flag set, skipping git commit for bulk import - ${titleSlug}`);
    return;
  }
  const forceCommit = !!data.forceCommit;
  if (gitEnabled && alreadyCommitted && !forceCommit) {
    dbg.log(`handleSolved(): already committed submission event, skipping auto-commit - ${submissionCommitKey}`);
  }
  if (gitEnabled && (forceCommit || !alreadyCommitted)) {
    ...
    try {
      ...
      for (const p of pendingProblems) {
        ...
        try {
          if (p.notes && typeof p.notes === "string" && p.notes.trim()) {
            const base = problemBase(p.id || p.titleSlug, p.canonical, settings);
            const notesPath = `${base}/notes.md`;
            if (!seenPaths.has(notesPath)) {
              seenPaths.add(notesPath);
              filesToCommit.push({ path: notesPath, content: p.notes });
            }
          }
        } catch (e) { /* ignore */ }
          ...
          if (slug) {
            ...
            if (Array.isArray(chats) && chats.length) {
              for (const chat of chats) {
                ...
                filesToCommit.push({ path: chatPath, content: JSON.stringify(chat, null, 2) });
              }
            }
          }
        } catch (e) { /* ignore */ }
      ...
      await pushToMirrors(filesToCommit, commitMsg, commitOpts, settings, primaryResult.target ? _targetKey(primaryResult.target) : "");
    } catch (err) {
  ...
  await Promise.allSettled(
    mirrors.map(async (mirror) => {
      ...
      try {
        ...
        dbg.warn(`pushToMirrors(): ✗ mirror commit failed - ${mirror.provider}/${mirror.repo}:`, e.message);
      }
    }),
  );
}
...
async function _maybeGenerateAISummary(settings) {
  try {
    ...
    if (count < SUMMARY_EVERY_N_COMMITS) {
      await Storage.setSettings({ ...settings, _commitsSinceLastSummary: count });
      return;
    }
    ...
    const topTopics = Object.entries(byTopic).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([t, n]) => `${t}(${n})`).join(", ");
    ...
    const { review, providerId } = await generateAIReview({ code: prompt, titleSlug: "_summary" }, settings);
    if (review) {
...
async function handleResyncCount() {
  ...
  try {
    ...
    dbg.warn(`handleResyncCount(): failed to fetch remote index:`, e?.message);
  }
...
async function handleResyncAll(mode = "bulk", commitType = "chore") {
  ...
  dbg.log(`handleResyncAll(): found ${missing.length} missing/drifted problem(s)`);
  if (missing.length === 0) {
  ...
  if (mode === "individual") {
    dbg.log(`handleResyncAll(): creating ${missing.length} individual backdated commit(s)`);
    ...
    for (const entry of historicalCommits.sort((a, b) => new Date(a.date) - new Date(b.date))) {
      dbg.log(`handleResyncAll(): committing (backdated to ${entry.date.toISOString()})`);
      await _commitWithFailover(entry.files, entry.message, entry.repoName, { date: entry.date }, settings);
    }
  } else {
    dbg.log(`handleResyncAll(): creating bulk atomic commit with ${missing.length} problem(s)`);
    ...
    filesToCommit.push({ path: "index.json", content: await buildIndexJson() });
    dbg.log(`handleResyncAll(): prepared ${filesToCommit.length} file(s) for bulk commit`);
    await _commitWithFailover(
      filesToCommit,
  ...
  const activeTarget = _normalizeGitTarget((await Storage.getSettings().catch(() => settings)).git_active_primary || _getDefaultPrimaryTarget(settings));
  await pushToMirrors(allFiles, "chore: sync " + missing.length + " problem(s) [CodeLedger]", {}, settings, activeTarget ? _targetKey(activeTarget) : "");

  dbg.log(`handleResyncAll(): sync complete - committed ${missing.length} problem(s)`);
...
async function handleBulkImport(problems = []) {
  ...
  try {
    const { compareSolutions, mergeSolutions } = await import("../core/ai-deduplication.js");
    // Group imported problems by titleSlug + lang
    const byKey = {};
    for (const p of problems) {
      const slug = p.titleSlug || (p.id || "").split("::")[0];
      const lang = (p.lang && (p.lang.name || p.lang.slug)) || (p.langName || p.lang) || "unknown";
    ...
    for (const k of Object.keys(byKey)) {
      ...
      for (let i = 1; i < group.length; i++) {
        try {
          const res = await compareSolutions((await Storage.getSettings()).aiProvider || "gemini", { code: primary.code, lang: primary.lang }, { code: group[i].code, lang: group[i].lang });
          if (res?.same) {
            // Mark older entry as duplicateOf primary
            const older = await Storage.getProblem(group[i].id).catch(() => null);
            if (older) {
              older.isDuplicate = true;
              older.duplicateOf = primary.id;
              await Storage.saveProblem(older).catch(() => { });
              // Queue for review
              await Storage.markPendingProblemKey(`dedup:${older.id}`).catch(() => { });
            }
          }
          // If AI thinks they're same, try to produce merged canonical solution
          if (res?.same) {
            try {
              const providerId = (await Storage.getSettings()).aiProvider || "gemini";
              const merged = await mergeSolutions(providerId, [primary, group[i]], primary.lang?.name || primary.lang || (group[i].lang && group[i].lang.name) || null);
              if (merged) {
                // Store merge proposal on the primary problem for review.
                const primaryProblem = await Storage.getProblem(primary.id).catch(() => null);
                if (primaryProblem) {
                  primaryProblem.aiMergePending = true;
                  primaryProblem.aiMergeOriginalCode = primaryProblem.code || "";
                  primaryProblem.aiMergeProposedCode = merged;
                  primaryProblem.aiMergeSources = [primary.id, group[i].id].filter(Boolean);
                  primaryProblem.methods = primaryProblem.methods || [];
                  primaryProblem.methods.push({ title: "AI-merge proposal", language: primary.lang?.name || primary.lang || "unknown", timestamp: Date.now() });
                  await Storage.saveProblem(primaryProblem).catch(() => { });
                  dbg.log(`handleBulkImport(): dedup - proposed merged solution for ${primaryProblem.titleSlug || primaryProblem.id}`);
                  // Mark the older entry as duplicate/removed
                  const older2 = await Storage.getProblem(group[i].id).catch(() => null);
                  if (older2) {
                    older2.isDuplicate = true;
                    older2.duplicateOf = primary.id;
                    await Storage.saveProblem(older2).catch(() => { });
                    await Storage.markPendingProblemKey(`dedup:${older2.id}`).catch(() => { });
                  }
                }
              }
            } catch (e) {
              dbg.warn(`handleBulkImport(): AI auto-merge failed:`, e?.message || e);
            }
          }
        } catch (e) {
          dbg.warn(`handleBulkImport(): bulk dedup compare failed:`, e?.message || e);
        }
      }
    }
  } catch (e) {
    dbg.warn(`handleBulkImport(): post-import dedup failed:`, e?.message || e);
  }
...
async function handleAIChat(messages, context = {}) {
  ...
  const systemPrompt = (skillsPrefix || "") + (knowledgeCtx ? knowledgeCtx + "\n\n" : "") + baseSystemPrompt;
  dbg.log(`handleAIChat(): systemPrompt built (${systemPrompt.length} chars, skills=${!!skillsPrefix}, kb=${!!knowledgeCtx})`);
  ...
  for (let idx = 0; idx < providers.length; idx++) {
    ...
    if (settings[`${provider.id}_enabled`] === false) {
      dbg.log(`handleAIChat(): attempt ${idx + 1}/${providers.length} - ${provider.id} disabled, skipping`);
      continue;
    }
    const ai = registry.getAIProvider(provider.id);
    if (!ai) {
      dbg.warn(`handleAIChat(): attempt ${idx + 1}/${providers.length} - ${provider.id} handler not found`);
      continue;
    }
    try {
      ...
      dbg.log(`handleAIChat(): received response from ${provider.id} (${String(response || "").length} chars)`);
      return response;
    } catch (e) {
      dbg.warn(`handleAIChat(): attempt ${idx + 1}/${providers.length} - ${provider.id} failed:`, e?.message);
    }
  }
...
async function handleRegenerateAIReview(problem = {}) {
  dbg.log(`handleRegenerateAIReview(): starting for ${problem.titleSlug || problem.id || 'unknown'}`);
  ...
  dbg.log(`handleRegenerateAIReview(): complete - committed=${commitResult?.committed || 0}`);
  return { problem: updated, review, providerId, committed: commitResult?.committed || 0 };
}
...
async function handleQueueAllAIReviews() {
  ...
  const allProblems = await Storage.getAllProblems();
  dbg.log(`handleQueueAllAIReviews(): checking ${allProblems.length} problem(s) for missing reviews`);
  ...
  dbg.log(`handleQueueAllAIReviews(): queued ${queued}/${toQueue.length} problem(s) for AI review`);
  return { queued };
}
...
async function processAIReviewQueue() {
  try {
    dbg.log(`processAIReviewQueue(): alarm tick - starting queue processing...`);
    ...
    while (item && processed < BATCH_SIZE) {
      dbg.log(`processAIReviewQueue(): item ${item.id} for problem ${item.problemId}`);
      await markProcessing(item.id);
      try {
        const problem = await Storage.getProblem(item.problemId);
        if (!problem) {
          await markDone(item.id);
          dbg.warn(`processAIReviewQueue(): problem ${item.problemId} not found in storage`);
          processed++;
        ...
        if (!problem.code) {
          await markDone(item.id);
          dbg.warn(`processAIReviewQueue(): problem ${item.problemId} has no code, skipping`);
          processed++;
          item = await getNextPendingReview();
          continue;
        }
        ...
        const settings = await Storage.getSettings();
        dbg.log(`processAIReviewQueue(): generating review (${processed + 1}/${BATCH_SIZE})`);
        const { review, providerId } = await generateAIReview(problem, settings);
        const updated = { ...problem, aiReview: review };
        await Storage.saveProblem(updated);
        ...
        await markDone(item.id);
        dbg.log(`processAIReviewQueue(): processed ${item.problemId} via ${providerId}`);
        ...
        if (!willRetry) {
          dbg.error(`processAIReviewQueue(): ${item.problemId} - max retries exceeded:`, e.message);
        } else {
          dbg.warn(`processAIReviewQueue(): ${item.problemId} - will retry:`, e.message);
        }
      }
      processed++;
      item = await getNextPendingReview();
    }
    dbg.log(`processAIReviewQueue(): tick complete - processed ${processed}/${BATCH_SIZE} items`);
  } catch (e) {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  dbg.log(`onMessage(): received msg.type=${msg.type} from ${sender?.id || sender?.tab?.id || sender?.url || 'unknown'}`);
  ...
  if (msg.type === "REPO_DIAGNOSTICS") {
    ...
    (async () => {
      try {
        const settings = await Storage.getSettings();
        const git = registry.getGitProvider(settings.gitProvider || "github");
        if (!git) { sendResponse({ ok: false, error: "No git provider configured" }); return; }
        const token = await git.getToken().catch(() => null);
        if (!token) { sendResponse({ ok: false, error: "Not authenticated — connect GitHub first" }); return; }
        const owner = settings.github_owner || settings.github_username;
        const repo = settings.github_repo || settings.gitRepo;
        if (!owner || !repo) { sendResponse({ ok: false, error: "Repo not configured" }); return; }

        const { detectRepoLayoutVersion } = await import("./migration-manager.js");
        const { getContents } = await import("../handlers/git/github/api-client.js");
        const { LAYOUT_VERSION } = await import("../core/path-builder.js");

        const checks = { owner, repo, currentLayoutVersion: LAYOUT_VERSION };
        ...
        checks.infraStatus = {};
        await Promise.all(INFRA_FILES.map(async (f) => {
          try {
  ...
  if (msg.type === "REPO_REPAIR") {
    ...
    (async () => {
      try {
        const { migrateRepo, resetRepo, forceRebuildRepo } = await import("./migration-manager.js");
        let result;
...
try {
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    ...
    if (msg && msg.type === "RESYNC_ALL") {
      dbg.log(`onMessage(RESYNC_ALL): mode=${msg.mode || 'bulk'} commitType=${msg.commitType || 'chore'}`);
    ...
    if (msg && msg.type === "SYNC_PREVIEW") {
      ...
      handleSyncPreview()
        .then((result) => {
          dbg.log(`onMessage(SYNC_PREVIEW): new=${result.remoteOnly?.length}, conflicts=${result.conflicts?.length}`);
          sendResponse({ ok: true, ...result });
        })
        .catch((e) => {
    ...
    if (msg && msg.type === "SYNC_APPLY_IMPORT") {
      dbg.log(`onMessage(SYNC_APPLY_IMPORT): importing ${(msg.problems || []).length} problem(s)...`);
    ...
    if (msg && msg.type === "REFRESH_METADATA") {
      dbg.log(`onMessage(REFRESH_METADATA): queuing ${(msg.problems || []).length} problem(s) for refresh...`);
    ...
    if (msg && msg.type === "REGENERATE_AI_REVIEW") {
      dbg.log(`onMessage(REGENERATE_AI_REVIEW): regenerating review for ${(msg.problem || msg.data || {}).titleSlug || 'unknown'}`);
    ...
    if (msg && msg.type === "LIST_GITHUB_BACKUPS") {
      ...
      (async () => {
        try {
          const settings = await Storage.getSettings();
          const git = registry.getGitProvider(settings.gitProvider || "github");
          if (!git) { sendResponse({ ok: false, error: "No git provider" }); return; }
          const token = await git.getToken().catch(() => null);
          const owner = settings.github_owner || settings.github_username || "";
          const repo = settings.github_repo || settings.gitRepo || "";
          if (!token || !owner || !repo) { sendResponse({ ok: false, error: "Not configured" }); return; }
          const { listBackups } = await import("../core/backup/backup-manager.js");
    ...
    if (msg && msg.type === "COMMIT_GITHUB_BACKUP_NOW") {
      ...
      (async () => {
        try {
          ...
          const { commitBackupToGitHub } = await import("../core/backup/backup-manager.js");
    ...
    if (msg && msg.type === "RESTORE_GITHUB_BACKUP") {
      ...
      (async () => {
        try {
          ...
          const repo = settings.github_repo || settings.gitRepo || "";
          if (!token || !owner || !repo) { sendResponse({ ok: false, error: "Not configured" }); return; }
          const { fetchBackupSnapshot } = await import("../core/backup/backup-manager.js");
          const snapshot = await fetchBackupSnapshot(owner, repo, msg.filePath, token);
          if (!snapshot?.problems) { sendResponse({ ok: false, error: "Invalid snapshot" }); return; }
