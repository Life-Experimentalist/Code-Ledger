...
function resolveLang(rawLang) {
    ...
    if (typeof rawLang === "string") {
        ...
        // Special-case: Pandas (often shown as a Python sub-language on LeetCode)
        ...
        // Python variants: prefer explicit Python3 when present
        ...
        return {
            verbose: LANG_VERBOSE[slug] || rawLang,
            slug,
            ext: langExt(slug),
        };
    }
...
Be concise. Max 200 words.`;
        ...
        return {
            ...
            fields: [
                ...
                },
                {
                    ...
                    default: true,
                    description:
                        "Add LeetCode's similar problems list to your commit.",
                },
                {
    ...
    async _handleOnDemandFetch(page) {
        try {
            ...
            const slug =
                page.slug || params.get("cl_fetch_id") || params.get("slug");
            ...
            const merged = {
                ...
                problemStatement:
                    meta?.content || existing?.problemStatement || null,
    ...
    _injectDetailSyncBtn(page) {
        ...
        this._syncBtnObserver.observe(document.body, {
            childList: true,
            subtree: true,
        });
    }
    ...
    async _injectProgressImportBtn() {
        ...
        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            ...
            if (anchor) {
                ...
                // Place inline with the progress header and align right for a cleaner look
    ...
    async _injectProfileActionButtons() {
        ...
        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            ...
            if (column) {
                ...
                actions.appendChild(
                    this._createProfileActionButton(
                        "Open CodeLedger Library",
                        () => {
                            try {
                                tabs.create({
                                    url: runtime.getURL("library/library.html"),
                                });
                            } catch (_) {}
                        }
                    )
    ...
    _createImportBtn(pageUsername) {
        ...
        btn.addEventListener("click", () =>
            this._runProfileImport(pageUsername, btn)
        );
        return btn;
    }
        ...
        try {
            ...
            try {
                ...
                if (apiRes.ok) {
                    ...
                    show(
                        `Problem index: ${Object.keys(diffMap).length} entries.`
                    );
                } else {
            ...
            while (true) {
                ...
                const raw =
                    pageData.submissions_dump || pageData.submissions || [];
                for (const s of raw) {
                    const titleSlug = s.title_slug || s.titleSlug || "";
                    const langSlug = (s.lang || "")
                        .toLowerCase()
                        .replace(/\s+/g, "");
                    const statusOk =
            ...
            // ── Phase 3: Keep all accepted submissions (one entry per submission).
            // We'll import all submissions so multi-language and multiple accepted submissions
            // per problem are preserved. Later a dedup pass (AI) will mark duplicates for review/merge.
            ...
            const needMeta = [
                ...new Set(
                    picks
                        .filter(
                            (s) =>
                                !diffMap[s.titleSlug] || !tagsMap[s.titleSlug]
                        )
                        .map((s) => s.titleSlug)
                ),
            ];
            ...
            if (needMeta.length > 0) {
                show(
                    `Fetching tags & descriptions for ${needMeta.length} problems…`
                );
            ...
            for (let i = 0; i < picks.length; i += BATCH) {
                ...
                if ((i / BATCH) % 5 === 0 || i + BATCH >= picks.length) {
                    show(
                        `Code ${Math.min(i + BATCH, picks.length)}/${picks.length}…`
                    );
                }
            }
            ...
            const bulkProblems = withCode.map((sub) => {
                ...
                const title =
                    titleMap[sub.titleSlug] || sub.title || sub.titleSlug;
            ...
            if (imported > 0) {
                ...
                // After import completes, ask background for a sync preview to detect any remote conflicts.
                try {
    ...
    _onSubmitFired() {
        ...
        this._resultPollTimer = setInterval(() => {
            ...
            const page = detectPage(window.location.pathname);
            this._processSubmission(page, false).catch((e) =>
    ...
    _readFloatingAITestFailures() {
        try {
            ...
            if (errorBanner)
                resultLines.push((errorBanner.textContent || "").trim());
            document
                .querySelectorAll(
    ...
    async _checkSubmission() {
        ...
        const page = detectPage(window.location.pathname);
        dbg.log(
            "[_checkSubmission] pageType=" + page.type + ", slug=" + page.slug
        );
        if (
            page.type !== PAGE_TYPES.PROBLEM &&
            page.type !== PAGE_TYPES.SUBMISSION
            ...
            dbg.log(
                "[_checkSubmission] Not a problem/submission page, returning"
            );
            return;
        }
        ...
        if (page.type === PAGE_TYPES.PROBLEM) {
            const visible = this._isAcceptedVisible();
            dbg.log(
                "[_checkSubmission] Problem page, accepted visible=" + visible
            );
            if (!visible) return;
        } else {
            dbg.log("[_checkSubmission] Submission detail page, will fetch");
        }
    ...
    _isAcceptedVisible() {
        ...
        const bySelector = this.safeQuery(
            SELECTORS.submission.successIndicator
        );
        if (bySelector && /accepted/i.test(bySelector.textContent || ""))
            return true;
    ...
    async _processSubmission(page, isManual) {
        ...
        try {
            ...
            if (page.type === PAGE_TYPES.SUBMISSION && page.submissionId) {
                const res = await this._gql(QUERIES.SUBMISSION_DETAIL, {
                ...
                if (!isManual && lastId === String(latest.id)) {
                    dbg.log(
                        "Skipping already-committed submission",
                        slug,
                        latest.id
                    );
                    return;
                ...
                if (!submission.code) {
                    const monacoCode = this._getCodeFromMonaco();
                    if (monacoCode)
                        submission = { ...submission, code: monacoCode };
                }
            ...
            dbg.log(
                "[_processSubmission] detectionId=" +
                    detectionId +
                    ", lastDetectedId=" +
                    this.lastDetectedId
            );
            if (!isManual && detectionId === this.lastDetectedId) {
            ...
            // Detect whether this submission's code differs from what's already saved.
            ...
            try {
                ...
                if (existingProblem) {
                    ...
                    // No existing problem — request review for new imports when enabled
                    submission._requestAIReview = true;
                }
            } catch (e) {
            ...
            this.emitSolved({
                ...
                topic: resolvePrimaryTopic(
                    meta?.topicTags?.map((t) => t.name) || []
                ),
                tags: meta?.topicTags?.map((t) => t.name) || [],
                canonical: canonical
                    ? {
                ...
                runtime:
                    submission.runtimeDisplay || submission.runtime || null,
                ...
                similar: (meta?.similarQuestionList || []).filter(
                    (q) => !q.isPaidOnly
                ),
