...
import { getCurrentUser as ghGetCurrentUser } from "../../handlers/git/github/api-client.js";
...
export function PanelGit({ settings, onSettingsChange, onSetupRepo }) {
    ...
    const effectiveToken = oauthToken || settings?.github_token || "";

    const handleConnect = () => {
        const authUrl = `${CONSTANTS.URLS.AUTH_WORKER}/auth/github`;
        const popup = window.open(authUrl, "OAuth", "width=600,height=700");
        if (!popup)
            alert("Please allow popups for this page to connect GitHub.");
    ...
    const handleSync = async (mode = "bulk") => {
        ...
        if (!gitEnabled)
            return flash(
                "Git commits are disabled — enable them first.",
                false
            );
        setSyncBusy(true);
        ...
        if (typeof chrome !== "undefined" && chrome.runtime?.id) {
            try {
                keepalivePort = chrome.runtime.connect({ name: "sync-keepalive" });
                syncPortRef.current = keepalivePort;
                keepalivePort.onMessage.addListener((msg) => {
                    if (msg?.type === "sync-progress") {
                        setSyncProgress({ current: msg.current, total: msg.total });
                    }
                });
        ...
        try {
            ...
            if (!git)
                throw new Error("Git provider not available. Try reloading.");
    ...
    const handleBackup = async () => {
        ...
        let keepalivePort = null;
        if (typeof chrome !== "undefined" && chrome.runtime?.id) {
            try { keepalivePort = chrome.runtime.connect({ name: "backup-keepalive" }); } catch (_) {}
        }
        try {
            await new Promise((resolve, reject) => {
                if (typeof chrome === "undefined" || !chrome.runtime?.id) {
                    ...
                    chrome.runtime.sendMessage(
                        ...
                        (res) => {
                            ...
                            const pending =
                                ...
                                res.pendingConflicts === 0
                                    ? res.pendingConflicts
                                    : (res.conflicts || []).length;
                            if (pending > 0) {
                                setImportMsg(
                                    `Imported ${remoteOnly.length} problem${remoteOnly.length !== 1 ? "s" : ""}. ${pending} conflict${pending !== 1 ? "s" : ""} detected.`
                                );
                            }
                        }
                    );
                } catch (e) {
                    /* SYNC_PREVIEW is optional */
                }
            }
        } catch (e) {
    ...
    const mirrors = Array.isArray(settings?.git_mirrors) ? settings.git_mirrors : [];
    ...
    const addMirror = () => {
        ...
        saveMirrors([
            ...mirrors,
            { id: Date.now().toString(36), provider: mirrorProvider, owner: mirrorOwner.trim(), repo: mirrorRepo.trim(), enabled: false },
        ]);
        setMirrorOwner("");
        setMirrorRepo("");
        ...
        <div class="space-y-6 w-full">
            <div>
                ...
                <p class="text-xs text-slate-500 mb-4">
                    Connect your repository and configure how solutions are
                    committed.
                </p>
            </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500 mt-0.5">
                            When enabled, each accepted solution is
                            automatically committed to your repository.
                        </p>
                    </div>
                    <button
                        onClick=${() =>
                            onSettingsChange("gitEnabled", !gitEnabled)}
            ...
            >
                ...
                <p class="text-[11px] text-slate-500">
                    Only GitHub is fully supported right now. GitLab and
                    Bitbucket are marked under construction and stay disabled
                ...
                <div class="flex gap-2">
                    ${GIT_PROVIDERS.length
                        ? GIT_PROVIDERS.map((gp) => {
                              const status =
                                  gp.status || CONSTANTS.FEATURE_STATUS.STABLE;
                              const statusMeta =
                                  STATUS_META[status] ||
                                  STATUS_META[CONSTANTS.FEATURE_STATUS.STABLE];
                              const disabled =
                                  status !== CONSTANTS.FEATURE_STATUS.STABLE;
                              return html`
                                  <button
                                      key=${gp.id}
                                      onClick=${() => {
                ...
                ${repoName
                    ? html`
                          <div class="flex items-center gap-3 p-3 rounded-lg bg-white/3 border border-white/8">
                              <div class="flex-1 min-w-0">
                                  <p class="text-[10px] text-slate-500 uppercase tracking-wider mb-0.5">Main repository</p>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-600 mt-0.5">
                            Optional. Every commit to the main repo is replicated here exactly. Disabled by default.
                        </p>
                    </div>
                    ${!showAddMirror
                        ? html`
                              <button
                                  onClick=${() => { setShowAddMirror(true); setMirrorOwner(""); setMirrorRepo(""); setMirrorCheckState(null); }}
                ...
                ${mirrors.length > 0
                    ? html`
                          <div class="space-y-2">
                              ${mirrors.map(
                                  (m, i) => html`
                                      <div key=${m.id || i} class="flex items-center gap-2 p-2.5 rounded-lg bg-white/3 border border-white/8">
                                          <span class="shrink-0 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest border ${
                                              m.provider === "github"
                                          ...
                                          <span class="flex-1 min-w-0 font-mono text-xs ${m.enabled === false ? "text-slate-600 line-through" : "text-slate-300"} truncate">
                                              ${m.owner}/${m.repo}
                                          </span>
                                          <button
                                              onClick=${() => toggleMirrorEnabled(i)}
                                              title=${m.enabled === false ? "Enable mirror" : "Disable mirror"}
                                              class="shrink-0 relative inline-flex h-4 w-7 items-center rounded-full border transition-colors ${
                                                  m.enabled === false
                                                      ? "bg-white/5 border-white/10"
                                                      : "bg-cyan-500/30 border-cyan-500/40"
                                              }"
                                          >
                                              <span class="inline-block h-2.5 w-2.5 rounded-full bg-white shadow transform transition-transform ${
                                                  m.enabled === false ? "translate-x-0.5" : "translate-x-4"
                                              }"></span>
                                          </button>
                                          <button
                          ...
                          <div class="space-y-2 p-3 rounded-lg border border-cyan-500/20 bg-cyan-500/3">
                              <div class="flex gap-2">
                                  ${MIRROR_PROVIDERS.map(
                                      (p) => html`
                                          <button
                                              key=${p.id}
                                              onClick=${() => { if (p.ready) { setMirrorProvider(p.id); setMirrorCheckState(null); checkMirrorRepo(mirrorOwner, mirrorRepo, p.id); } }}
                                              disabled=${!p.ready}
                                              title=${p.ready ? p.label : `${p.label} — coming soon`}
                                              class="flex-1 py-1 rounded text-[11px] font-medium border transition-colors ${
                                                  mirrorProvider === p.id
                                                      ? "bg-cyan-500/20 border-cyan-500/40 text-cyan-200"
                                                      : "bg-white/5 border-white/10 text-slate-500"
                                              } disabled:opacity-40 disabled:cursor-not-allowed"
                                          >
                                              ${p.label}${!p.ready ? html`<span class="ml-1 text-[8px] opacity-60">soon</span>` : ""}
                                          </button>
                                      `
                                  )}
                              </div>
                              <div class="flex gap-2">
                                  <input
                                      ...
                                      onInput=${(e) => { setMirrorOwner(e.target.value); checkMirrorRepo(e.target.value, mirrorRepo, mirrorProvider); }}
                                  ...
                                  <input
                                      ...
                                      onInput=${(e) => { setMirrorRepo(e.target.value); checkMirrorRepo(mirrorOwner, e.target.value, mirrorProvider); }}
                                      class="flex-1 px-2.5 py-1.5 bg-black border border-white/10 rounded-lg text-xs text-slate-300 placeholder-slate-600 focus:outline-none focus:border-cyan-500/40"
                                  />
                              </div>
                              <div class="flex items-center justify-between gap-2">
                                  <span class="text-[10px] ${
                                      mirrorCheckState === "ok" ? "text-emerald-400" :
                                      mirrorCheckState === "err" ? "text-rose-400" :
                                      mirrorCheckState === "checking" ? "text-slate-500" :
                                      "text-transparent"
                                  }">
                                      ${mirrorCheckState === "ok" ? "✓ Repo accessible" :
                                        mirrorCheckState === "err" ? "✗ Repo not found or no access" :
                                        mirrorCheckState === "checking" ? "Checking…" : "—"}
                                  </span>
                                  <div class="flex gap-2">
                                      <button
                                          onClick=${() => { setShowAddMirror(false); setMirrorCheckState(null); }}
                                      ...
                                      <button
                                          onClick=${addMirror}
                                          disabled=${!mirrorOwner.trim() || !mirrorRepo.trim() || mirrorCheckState === "err"}
            ...
            <div class="p-4 rounded-xl border border-white/8 bg-white/2 space-y-3">
                ...
                <p class="text-[11px] text-slate-500">
                    Pulls any new problems from the repository into the local library, then pushes all unsynced local solutions — one commit per problem, backdated to the original solve time.
                    ${syncCount !== null && syncCount > 0
                        ? html`<span class="text-amber-400"> ${syncCount} problem${syncCount !== 1 ? "s" : ""} pending.</span>`
                        : ""}
                </p>
                ${syncProgress
                    ? html`<div class="flex items-center gap-2">
                        <div class="flex-1 h-1 rounded-full bg-white/8 overflow-hidden">
                            <div class="h-full bg-cyan-500 transition-all duration-300 rounded-full"
                ...
                <div class="flex gap-2 flex-wrap">
                    ...
                    >
                        ${syncBusy && syncProgress
                            ...
                            : syncBusy ? "Syncing…" : "Sync to GitHub"}
                    </button>
                    <button
                        onClick=${handleBackup}
                        disabled=${backupBusy || !gitEnabled || activeProvider?.status !== CONSTANTS.FEATURE_STATUS.STABLE}
                ...
                ${activeProvider?.status && activeProvider.status !== CONSTANTS.FEATURE_STATUS.STABLE
                    ? html`<p class="text-[11px] text-amber-400">${STATUS_META[activeProvider.status]?.label || "This provider"} is not ready yet.</p>`
                    : ""}
                ${msg.text && html`<p class="text-xs ${msg.ok ? "text-emerald-400" : "text-rose-400"}">${msg.text}</p>`}
            </div>
            ...
            ${settings._pendingConflicts > 0
                ? html`
                      <div
                          class="flex items-center gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20"
                      >
                          <span class="text-amber-400 text-sm"
                              >⚠ ${settings._pendingConflicts}
                              conflict${settings._pendingConflicts !== 1
                                  ? "s"
                                  : ""}
                              detected during background sync.</span
                          >
                          <button
                              onClick=${handleImport}
                              class="text-xs text-amber-300 underline hover:no-underline"
                          >
                              Resolve now
                          </button>
                      </div>
            ...
            ${repoName
                ? html`
                      ...
                      >
                          ...
                          <div class="flex items-center justify-between">
                              <div>
                                  ...
                                  <p class="text-xs text-slate-500">
                                      Pull all problems from your connected repo
                          ...
                          ${importMsg
                              ? html`<p class="text-xs text-cyan-400">
                                    ${importMsg}
                                </p>`
                              : ""}
                      </div>
                ...
                ? html`
                      <${ConflictResolutionModal}
                          ...
                          onResolve=${async (resolved) => {
                              ...
                              setImportData(null);
                              setImportMsg(
                                  `Imported ${resolved.length} problem${resolved.length !== 1 ? "s" : ""}`
                              );
                              const s = await Storage.getSettings();
                              await Storage.setSettings({
                                  ...s,
                              ...
                              if (syncNeedsPush) {
                                  ...
                                  await new Promise((resolve, reject) => {
                                      ...
                                      setImportMsg(
                                          `Resolved conflicts, but repo push failed: ${e.message}`
                                      )
                                  );
                              }
                          }}
