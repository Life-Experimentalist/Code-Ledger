        ...
        >
            <div>
                ...
                <p class="text-[11px] text-slate-500">
                    After each commit to the primary repository, the same files
                    are pushed to every mirror. If the active primary target
                    fails, mirrors are attempted in the order shown below and
                    the first healthy target becomes active. Mirrors require the
                    provider's token to be configured in its settings section.
                </p>
            </div>
                      ...
                      <div class="flex flex-col gap-2">
                          ${mirrors.map(
                              (m, i) => html`
                                  <div
                                      key=${i}
                                      class="flex items-center gap-3 p-2.5 rounded-xl bg-white/5 border border-white/10"
                                  >
                                      <span
                                          class="text-[10px] font-medium text-cyan-400 uppercase tracking-wider w-16 shrink-0"
                                          >${providerLabel(m.provider)}</span
                                      >
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <input
                        ...
                        value=${draft.repo}
                        onInput=${(e) =>
                            setDraft((d) => ({ ...d, repo: e.target.value }))}
