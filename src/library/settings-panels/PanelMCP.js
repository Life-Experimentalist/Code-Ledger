...
export function PanelMCP() {
    ...
    if (!config) {
        return html`<div class="p-4 text-slate-400">Loading MCP config...</div>`;
    }
    ...
    <div class="flex flex-col gap-6 p-4">
      ...
      <div class="bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-lg p-4 border border-blue-500/20">
        ...
        <p class="text-sm text-slate-300 leading-relaxed">
          AI providers can automatically invoke these tools to provide richer analysis. Enable/disable individual
          tools based on your preferences.
        </p>
      </div>

      <!-- Global Settings -->
      <div class="space-y-3">
        <h4 class="text-sm font-semibold text-slate-200">Global Settings</h4>

        <label class="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 border border-slate-700 cursor-pointer hover:bg-slate-800">
          <input
            type="checkbox"
      ...
      <div class="text-xs text-slate-500 pt-2 border-t border-slate-700">
        Enabled Tools: ${enabledIds.size} / ${Object.keys(MCP_TOOL_INFO).length}
      </div>
    </div>
  `;
}
