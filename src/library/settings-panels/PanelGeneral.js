  ...
  return html`
    <div class="space-y-6 max-w-xl">
      <div>
        ...
        <p class="text-xs text-slate-500 mb-4">Customize the color theme and display preferences.</p>
      </div>

      <!-- Mode pills -->
      <div>
        <label class="block text-xs font-medium text-slate-400 mb-2">Color mode</label>
        <div class="flex gap-2">
          ${MODE_OPTIONS.map(({ id, label }) => html`
            <button
              key=${id}
      ...
      <!-- Auto behavior -->
      ${theme.mode === "auto" && html`
        <div>
          <label class="block text-xs font-medium text-slate-400 mb-2">Auto-switch behavior</label>
          <div class="space-y-1.5">
            ${AUTO_BEHAVIOR_OPTIONS.map(({ id, label }) => html`
          ...
          ${theme.autoBehavior === "schedule" && html`
      ...
      <!-- Preset skin -->
      <div>
        <label class="block text-xs font-medium text-slate-400 mb-2">Preset skin</label>
        <select
        ...
        >
          ${Object.entries(presetsByGroup).map(([group, items]) => html`
      ...
      <div class="rounded-xl border border-white/10 overflow-hidden">
        <div class="px-4 py-2 bg-[var(--cl-bg,#050508)] flex items-center gap-3">
          <span class="w-3 h-3 rounded-full bg-[var(--cl-accent,#06b6d4)]"></span>
          <span class="text-xs text-[var(--cl-text,#e2e8f0)]">Preview strip</span>
          <span class="ml-auto text-[10px] text-[var(--cl-text-muted,#64748b)]">accent · bg · text</span>
        </div>
      </div>

      <!-- Accent color override -->
      <div>
        <label class="block text-xs font-medium text-slate-400 mb-2">Accent color override</label>
        <div class="flex items-center gap-3">
          <input
            type="color"
            value=${theme.accentPrimary || (THEME_PRESETS[theme.preset]?.dark?.["primary-color"] || "#06b6d4")}
          ...
          <span class="text-xs text-slate-400">
            ${theme.accentPrimary ? theme.accentPrimary : "Using preset default"}
          </span>
          ${theme.accentPrimary && html`
            <button
      ...
      <div>
        <label class="block text-xs font-medium text-slate-400 mb-2">Background color override</label>
        <div class="flex items-center gap-3">
          <input
            type="color"
            value=${theme.bgColor || (THEME_PRESETS[theme.preset]?.dark?.["bg-primary"] || "#050508")}
      ...
      <div class="flex items-center gap-3 pt-2">
        ...
        >${saving ? "Saving…" : "Apply & save"}</button>
        ${saveMsg && html`
          <span class="text-xs ${saveMsg.includes("failed") ? "text-rose-400" : "text-emerald-400"}">${saveMsg}</span>
        `}
      </div>
    </div>
  `;
