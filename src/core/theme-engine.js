  ...
  try {
    const tz =
      typeof Intl !== "undefined" && Intl.DateTimeFormat
        ...
        : "";
    const southern =
      /^(America\/(Sao_Paulo|Argentina)|Australia|Pacific\/(Auckland|Fiji)|Africa\/(Johannesburg|Nairobi))/i.test(
        tz,
      );
...
export function setThemeVariables(theme) {
  ...
  if (typeof window !== "undefined" && window.matchMedia) {
    ...
    if (window.__clOSThemeFn) {
      try {
        mq.removeEventListener("change", window.__clOSThemeFn);
      } catch (_) {}
  ...
  if (typeof window !== "undefined") {
    if (window.__clTimerHandle) {
      clearInterval(window.__clTimerHandle);
      window.__clTimerHandle = null;
    }
    if (
      theme?.mode === "auto" &&
