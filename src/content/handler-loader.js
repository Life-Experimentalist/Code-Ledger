...
async function loadHandler() {
  ...
  try {
    ...
    console.warn(`[CodeLedger:HandlerLoader] loadHandler(): debug init failed (non-blocking):`, e?.message);
  }
  ...
  try {
    if (isHost("leetcode.com", hostname)) {
      ...
      console.log(`[CodeLedger:HandlerLoader] loadHandler(): hostname not recognized (${hostname}) — CodeLedger not applicable`);
    }
  } catch (err) {
    console.error(`[CodeLedger:HandlerLoader] loadHandler(): ✗ handler initialization failed:`, err?.message || err);
  }
}
