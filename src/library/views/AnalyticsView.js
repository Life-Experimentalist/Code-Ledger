...
export function AnalyticsView({ problems }) {
  ...
  const stats = useMemo(() => {
    ...
    problems.forEach((p) => {
      ...
      p.tags.forEach((t) => {
        if (!s.topics[t])
          s.topics[t] = { easy: 0, medium: 0, hard: 0, total: 0, score: 0 };
        s.topics[t].total++;
        if (cat === "Easy") {
          s.topics[t].easy++;
          s.topics[t].score += 1;
        ...
        datasets: [
          {
            label: "Mastery (%)",
            data: tpLabels.map((t) =>
              Math.min(100, (stats.topics[t].score / MAX_TOPIC_SCORE) * 100),
            ),
        ...
        datasets: [
          {
            ...
            backgroundColor: Object.keys(stats.platforms).map(
              (p) => platformColors[p.toLowerCase()] || "#94a3b8",
            ),
          },
        ],
      },
  ...
  const unsolvedNext = useMemo(() => {
    ...
    if (hardRatio > 0.3) {
      ...
      recommendations.push(
        ...available.filter((p) => p.diff !== "Easy").slice(0, 3),
      );
    } else if (medRatio > 0.4) {
      recommendations.push(
        ...available.filter((p) => p.diff === "Medium").slice(0, 2),
      );
      recommendations.push(
        ...available.filter((p) => p.diff === "Hard").slice(0, 1),
      );
    } else {
      recommendations.push(
        ...available
          .filter((p) => p.diff === "Easy" || p.diff === "Medium")
          .slice(0, 3),
      );
    }
  ...
  return html`
    <div class="flex flex-col gap-6 w-full pb-10">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
          class="p-6 bg-[#0a0a0f] border border-white/5 rounded-2xl flex flex-col gap-4 relative overflow-hidden h-64"
        >
          <div
          ...
          <div class="flex-1 flex justify-center items-center z-10 relative">
            ...
            />
            <div
              class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-6"
            >
              ...
              ([topic, counts]) => html`
                ...
                >
                  <div class="flex justify-between items-center mb-3">
                    <span
                      class="font-medium text-slate-300 group-hover:text-cyan-400 transition-colors"
                      >${topic}</span
                    >
                    <span class="text-xs font-mono text-slate-500"
                      >${counts.total} / 50</span
                    >
                  </div>
                  <div
                  ...
                  >
                    <div
                      ...
                      style=${{ width: `${(counts.easy / 50) * 100}%` }}
                    ></div>
                    <div
                      class="h-full bg-amber-500"
                      style=${{ width: `${(counts.medium / 50) * 100}%` }}
                    ></div>
                    <div
                      class="h-full bg-rose-500"
                      style=${{ width: `${(counts.hard / 50) * 100}%` }}
                    ></div>
                  </div>
                </div>
              `,
        ...
        <div class="flex flex-col gap-4">
          ...
          >
            <p class="text-xs text-slate-400 mb-2">
              Based on what you've solved, try these next:
            </p>
            ${unsolvedNext.map(
              (rec) => html`
                <a
                ...
                >
                  <div class="flex flex-col">
                    <span
                      class="text-sm font-medium text-slate-300 group-hover:text-cyan-400"
                      >${rec.title}</span
                    >
                  ...
                  <span
                    class="text-xs px-2 py-0.5 rounded-full ${rec.diff ===
                    "Hard"
                      ? "bg-rose-500/10 text-rose-400"
                      : "bg-amber-500/10 text-amber-400"} border border-current/10"
                  >
                    ${rec.diff}
                  </span>
                </a>
