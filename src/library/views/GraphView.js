...
function applyCircularLayout(nodes, edges) {
    ...
    topicNodes.forEach((n, i) => {
        ...
        seedNode(
            n,
            Math.cos(angle) * topicRadius,
            Math.sin(angle) * topicRadius
        );
...
function applyLayeredLayout(nodes, edges) {
    ...
    for (const [topicId, bucket] of bucketsByTopic) {
        const baseX =
            topicId === "__orphan__" ? 0 : topicPos.get(topicId)?.x || 0;
...
function drawGraph(ctx, nodes, edges, transform, hovered, selected) {
    ...
    const drawableIds = new Set(
        showGhostNodes
            ? nodes.map((n) => n.id)
            : nodes
                  .filter((n) => n.type === "topic" || n.solved)
                  .map((n) => n.id)
    );
...
export function GraphView({
    ...
    useEffect(() => {
        ...
        setStats({
            ...
            solved: newNodes.filter((n) => n.type === "problem" && n.solved)
                .length,
    ...
    function getVisibleGraphData() {
        ...
        for (const n of nodes) {
            ...
            if (diff !== "All" && String(n.difficulty || "Unknown") !== diff)
                continue;
            if (platform !== "All" && String(n.platform || "") !== platform)
                continue;
            if (topic !== "All") {
                const tags = n.tags || [];
        ...
        for (const e of edges) {
            if (e.type === "topic-problem") continue;
            if (
                visibleProblemIds.has(e.source) &&
                visibleProblemIds.has(e.target)
            ) {
    ...
    const topicCounts = useMemo(() => {
        const map = new Map();
        for (const n of simRef.current.nodes.filter(
            (n) => n.type === "problem"
        )) {
        ...
        if (hit) {
            if (isGroupDrag) {
                const depthLimit = Math.max(
                    1,
                    Number(groupDepthRef.current || 1)
                );
        ...
        return html`
            <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    ${node.lang
                        ? html`<span
                              class="text-[10px] font-mono text-cyan-500/70"
                              >${node.lang}</span
                          >`
                        : ""}
                </div>
                                    ...
                                    >
                                        ${node.runtime
                                            ? html`<span class="text-slate-500"
                                                      >Runtime</span
                                                  ><span
                                                      class="text-slate-200 text-right"
                                                      >${node.runtime}${node.runtimePct
                                                          ? html` <span
                                                                class="text-cyan-500/60"
                                                                >·
                                                                ${node.runtimePct.toFixed(
                                                                    0
                                                                )}%</span
                                                            >`
                                                          : ""}</span
    ...
    return html`
        <div class="flex flex-col gap-4 w-full h-full min-h-[600px]">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <div class="flex items-center gap-2 ml-2">
                    ...
                    <select
                        ...
                        onChange=${(e) =>
                            setFilterDifficultyGraph(e.target.value)}
                        class="px-2 py-1.5 bg-black border border-white/10 rounded text-xs text-slate-300"
                    >
                    ...
                    <select
                        ...
                        onChange=${(e) =>
                            setFilterPlatformGraph(e.target.value)}
                        class="px-2 py-1.5 bg-black border border-white/10 rounded text-xs text-slate-300"
                    >
                    ...
                    <div class="flex items-center gap-1 text-xs text-slate-400">
                        <select
                            ...
                            onChange=${(e) =>
                                setFilterTopicGraph(e.target.value)}
                        ...
                        >
                            ...
                            <input
                                ...
                                value=${groupDepth}
                                onInput=${(e) =>
                                    setGroupDepth(Number(e.target.value) || 1)}
                                class="w-20"
                            />
            ...
            >
                ...
                >
                    ...
                    <div class="flex items-center gap-2">
                        <span
                            class="w-3 h-3 rounded-full bg-[#22c55e] inline-block"
                        ></span
                        >Easy
                    </div>
                    <div class="flex items-center gap-2">
                        <span
                            class="w-3 h-3 rounded-full bg-[#f59e0b] inline-block"
                        ></span
                        >Medium
                    </div>
                    <div class="flex items-center gap-2">
                        <span
                            class="w-3 h-3 rounded-full bg-[#ef4444] inline-block"
                        ></span
                        >Hard
                    </div>
                    <div class="flex items-center gap-2">
                        <span
                            class="w-3 h-3 rounded-full bg-[#64748b] inline-block"
                        ></span
                        >Unknown
                    </div>
                    <div class="flex items-center gap-2">
            ...
            <p class="text-[10px] text-slate-600 text-center">
                Drag nodes · scroll or +/− to zoom · ▣ fit view · hover to
                preview · click to pin · double-click drag to move a cluster
            </p>
