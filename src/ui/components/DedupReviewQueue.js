...
export function DedupReviewQueue({ onClose = () => { } }) {
    ...
    async function loadQueue() {
        ...
        setQueue((problems || []).filter((p) => p?.aiMergePending && p?.aiMergeProposedCode));
        } catch (e) {
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative w-full max-w-3xl mx-4 bg-slate-900 p-6 rounded-lg border border-white/10 max-h-96 overflow-auto">
        <div class="flex items-center justify-between gap-4 mb-4">
          <h3 class="text-lg font-semibold">Deduplication Review Queue</h3>
          <button onClick=${onClose} class="text-slate-400 hover:text-white">Close</button>
        </div>

        ${loading ? html`<div class="text-sm text-slate-400">Loading…</div>` :
            queue.length === 0
                ? html`<div class="text-sm text-slate-400">No duplicates detected. All solutions look unique!</div>`
                : html`
              <div class="space-y-3">
                ${queue.map((item) => html`
                  <div class="p-4 bg-white/3 border border-white/5 rounded-lg">
                    <div class="flex items-start justify-between gap-4">
                      <div>
                        <div class="font-medium text-sm">${item.title}</div>
                        <div class="text-xs text-slate-400 mt-1">AI merge proposal · ${item.lang?.name || item.lang?.slug || item.lang || "unknown language"}</div>
                        <div class="text-xs text-slate-500 mt-2 space-y-1">
                          ...
                          <div>Sources: ${(item.aiMergeSources || []).join(", ") || "n/a"}</div>
                        </div>
                      </div>
                      <div class="flex gap-2">
                        ...
                        >${processingId === item.id ? "Applying…" : "Approve"}</button>
                        <button
