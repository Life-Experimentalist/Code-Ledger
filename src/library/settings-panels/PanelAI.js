...
export function PanelAI({ settings, onSettingsChange }) {
    ...
    const primaryProvider =
        settings?.aiProvider || CONSTANTS.AI_DEFAULT_PRIMARY;
    ...
    const saveKeys = async (providerId) => {
        ...
        try {
            const all = { ...savedKeys };
            const existing = Array.isArray(all[providerId])
                ? all[providerId]
                : [];
        ...
        <div class="space-y-6 w-full">
            <div>
                ...
                <p class="text-xs text-slate-500 mb-4">
                    Configure AI providers for code review and chat. Providers
                    are disabled by default until you add a key.
                </p>
            </div>

            <!-- Auto-review toggle -->
            <div
                class="p-4 rounded-xl border border-white/8 bg-white/2 space-y-2"
            >
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500">
                            Automatically run AI review each time a solution is
                            committed to GitHub.
                        </p>
                    </div>
                    <button
                        onClick=${() =>
                            onSettingsChange("autoReview", !autoReview)}
            ...
            >
                ...
                <div class="space-y-2">
                    ...
                    <select
                        ...
                        onChange=${(e) =>
                            onSettingsChange("aiProvider", e.target.value)}
                ...
                <div class="space-y-2">
                    ...
                    <select
                        ...
                        onChange=${(e) =>
                            onSettingsChange("aiSecondary", e.target.value)}
                        class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-slate-300 focus:outline-none focus:border-cyan-500/40"
                    >
                    ...
                    <p class="text-[11px] text-cyan-400/80">
                        You can reuse the same provider as primary if the
                        fallback model is different.
                    </p>
                    ${fallbackProvider &&
                    html`
                        <${ModelSelector}
                            ...
                            onSelect=${(v) =>
                                onSettingsChange("aiSecondaryModel", v)}
                            endpoint=${settings?.[
                                `${fallbackProvider}_endpoint`
                            ] || ""}
            ...
            <div class="space-y-3">
                ...
                ${PROVIDERS.map((p) => {
                    ...
                    const keys = Array.isArray(savedKeys[p.id])
                        ? savedKeys[p.id]
                        : [];
                    ...
                    const defaultEp =
                        CONSTANTS.AI_PROVIDERS[p.id]?.endpoint || "";
