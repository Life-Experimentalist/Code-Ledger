...
function renderTable(block) {
  ...
  const th = headers
    .map((h) => `<th class="px-3 py-1.5 text-left text-[11px] font-semibold text-slate-300 border-b border-white/10 whitespace-nowrap">${escapeHtml(h)}</th>`)
    .join("");
  const trs = rows
    .map(
      (row) =>
        `<tr class="border-b border-white/5 hover:bg-white/[0.02]">${row
          .map((cell) => `<td class="px-3 py-1.5 text-xs text-slate-300">${renderInline(escapeHtml(cell))}</td>`)
          .join("")}</tr>`,
    )
    .join("");
...
export function parseMarkdown(text) {
  ...
  t = t.replace(/`([^`\n]+)`/g, (_, code) =>
    S(`<code class="px-1 py-0.5 rounded bg-white/10 text-cyan-300 text-[0.85em] font-mono">${escapeHtml(code)}</code>`),
  );
  ...
  t = t
    ...
    .replace(/^> (.+)$/gm, '<blockquote class="pl-3 border-l-2 border-cyan-500/40 text-slate-400 italic my-1">$1</blockquote>')
    .replace(/\*\*(.+?)\*\*/g, '<strong class="text-white font-semibold">$1</strong>')
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" rel="noopener" class="text-cyan-400 hover:text-cyan-300 underline">$1</a>')
    .replace(/^[-*] (.+)$/gm, '<li class="ml-4 list-disc">$1</li>')
    .replace(/((?:<li class="ml-4 list-disc">[\s\S]+?<\/li>\n?)+)/g, '<ul class="my-2 space-y-0.5 text-slate-300 list-disc list-inside">$1</ul>')
    .replace(/^\d+\. (.+)$/gm, '<li class="ml-4 list-decimal">$1</li>')
    .replace(/((?:<li class="ml-4 list-decimal">[\s\S]+?<\/li>\n?)+)/g, '<ol class="my-2 space-y-0.5 text-slate-300 list-decimal list-inside">$1</ol>')
  ...
  return html`
    ...
    ${copyPrompt && html`
      <div
        ...
        style=${{ left: `${copyPrompt.x}px`, top: `${copyPrompt.y + 10}px` }}
      >
