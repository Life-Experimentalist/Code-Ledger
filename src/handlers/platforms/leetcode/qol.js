...
/** Build the AI panel toggle button for the editor toolbar. */
function makeAIBtn(onClickFn) {
  const btn = document.createElement("button");
  btn.id = "cl-ai-toolbar-btn";
  btn.title = "Open AI Assistant (Alt+`)  — CodeLedger";
  btn.className =
    "relative inline-flex gap-2 items-center justify-center font-medium cursor-pointer focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 transition-colors bg-transparent enabled:hover:bg-fill-secondary enabled:active:bg-fill-primary text-caption rounded text-text-primary group aspect-1 h-full p-1";
  btn.innerHTML = `<div class="relative text-[14px] leading-[normal] p-[1px] before:block text-sd-muted-foreground flex items-center justify-center">
    <svg aria-hidden="true" focusable="false" class="h-3.5 w-3.5" viewBox="0 0 576 512" xmlns="http://www.w3.org/2000/svg" fill="currentColor">
 ...
 *
 * @param {{ showCopy?: boolean, showPaste?: boolean, showAI?: boolean, onAIClick?: () => void }} opts
 */
export function injectQoL(opts = {}) {
  ...
  // If nothing is enabled, clean up and bail
  if (!showCopy && !showPaste && !showAI) {
    ["cl-code-copy", "cl-code-paste", "cl-ai-toolbar-btn"].forEach((id) =>
  ...
  const aiOk =
    !showAI ||
    (document.getElementById("cl-ai-toolbar-btn") &&
      document.getElementById("cl-ai-toolbar-btn").isConnected);
  if (copyOk && pasteOk && aiOk) {
    _qolInjected = true;
    return;
  }
  ...
  if (toolbar) {
    ...
    if (!showAI) document.getElementById("cl-ai-toolbar-btn")?.remove();
    _qolInjected = true;
    return;
  }
