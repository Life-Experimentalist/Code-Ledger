...
function PlatformBadge({ platform }) {
    ...
    return html`
        <span
            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-[10px] text-slate-400"
        >
            ${favicon
                ? html`<img
                      src=${favicon}
...
export function CanonicalView({ problems }) {
    ...
    const handleSearch = useCallback(() => {
        ...
        if (!q || !canonicalMap?.entries) {
            setSearchResults(null);
            return;
        }
        const hits = canonicalMap.entries.filter(
            (e) =>
    ...
    const handleSubmit = async () => {
        if (!githubToken) {
            setSubmitResult({
                ok: false,
                msg: "Connect GitHub first (in Settings) to submit canonical requests.",
            });
            return;
        }
        if (!canonTitle.trim()) {
        ...
        if (validAliases.length < 2) {
            setSubmitResult({
                ok: false,
                msg: "At least two platform slugs are required.",
            });
            return;
        }

        // Check if mapping already exists in canonical-map
        const existing = canonicalMap?.entries?.find(
            (e) =>
                e.canonicalTitle?.toLowerCase() ===
                    canonTitle.trim().toLowerCase() ||
                validAliases.some((a) =>
                    e.aliases?.some(
                        (ea) =>
                            ea.platform === a.platform &&
                            ea.slug === a.slug.trim()
                    )
                )
        );
        ...
        <div class="flex flex-col gap-8 w-full">
            ...
            <div class="flex flex-col gap-1">
                ...
                <p class="text-sm text-slate-400">
                    ...
                    Codeforces. Mappings with ${VOTES_REQUIRED}+ 👍 are merged
                ...
                ${!githubToken
                    ? html`
                          ...
                          >
                              ⚠️ Connect GitHub in <strong>Settings</strong> to
                              submit requests and vote.
                          </div>
                      `
                    : html`
                          <div class="mt-2 text-[11px] text-emerald-400">
                              ✓ GitHub connected — you can submit and vote.
                          </div>
                      `}
            </div>
            ...
            >
                    ...
                    ${searchQuery
                        ? html`
                              <button
                                  ...
                                  class="px-3 py-2 text-slate-500 hover:text-slate-300 border border-white/10 rounded-lg text-sm"
                              >
                                  ✕
                              </button>
                          `
                ...
                ${mapLoading
                    ...
                    : ""}
                ${mapError
                    ? html`<p class="text-[11px] text-rose-400">
                          Error: ${mapError}
                      </p>`
                    : ""}
                ${searchResults !== null
                          ...
                          >
                              ${searchResults.length === 0
                                  ...
                                  : searchResults.map(
                                        (entry) => html`
                                            <div
                                                class="p-3 bg-white/3 border border-white/5 rounded-xl flex flex-col gap-2"
                                            >
                                                <div
                                                ...
                                                >
                                                    <span
                                                        ...
                                                        >${entry.canonicalTitle}</span
                                                    >
                                                    <span
                                                        class="text-[10px] text-slate-500 px-1.5 py-0.5 bg-white/5 rounded"
                                                        >${entry.topic}</span
                                                    >
                                                </div>
                                                <div
                                                    ...
                                                    ? html`
                                                          ...
                                                          >
                                                              ${entry.tags.map(
                                                                  (t) => html`
                                                                      <span
                                                                          class="px-1.5 py-0.5 rounded text-[9px] bg-white/5 text-slate-500"
                                                                          >${t}</span
                                                                      >
                                                                  `
                                                              )}
                    ...
                    : !mapLoading && !mapError && !searchQuery
                      ? html`
                            <p class="text-[11px] text-slate-600">
                                ${canonicalMap?.entries?.length ?? 0} canonical
                                entries loaded. Search to find a mapping.
                            </p>
                        `
                      : ""}
            </div>
            ...
            >
                ...
                ${submitResult
                    ? html`
                          <div
                              class="text-[11px] px-3 py-2 rounded-lg ${submitResult.ok
            ...
            >
                ...
                ${!issuesLoading && !issuesError && issues.length === 0
                    ? html`
                          <p class="text-[11px] text-slate-600 italic">
                              No pending canonical requests. Be the first to
                              submit one!
                          </p>
                      `
                    : ""}
                ${issues.map((issue) => {
                    ...
                    return html`
                        <div
                            class="p-4 border rounded-xl flex flex-col gap-2 transition-colors ${ready
                ...
                <p class="text-[10px] text-slate-700 text-center">
                    Issues on
                    <a
                        href="https://github.com/${CANONICAL_REPO}"
                        target="_blank"
                        rel="noopener"
                        class="text-slate-600 hover:text-slate-400 underline"
                        >${CANONICAL_REPO}</a
                    >
