...
export const BUILTIN_SKILLS = [
    {
        ...
        system_prompt_modifier: "Adopt a Socratic teaching style. Ask probing questions to guide the user toward the answer instead of giving it directly. Only provide the answer if the user has tried multiple times.",
        auto_tools: [],
    },
    {
        id: "hint-giver",
        ...
        trigger: "on_stuck",
        system_prompt_modifier: "The user seems stuck. Give only one small, incremental hint that nudges them in the right direction without revealing the full approach. Do not spoil the solution.",
        conditions: { keywords: ["stuck", "hint", "help me", "i don't know", "no idea"] },
    },
    {
        id: "code-reviewer",
        name: "Code Reviewer",
        ...
        system_prompt_modifier: "Act as an expert code reviewer. Analyse the solution for: time complexity, space complexity, edge cases, readability, and potential optimisations. Be precise and constructive.",
    },
    {
        id: "next-problem",
        ...
        trigger: "after_solve",
        system_prompt_modifier: "The user has just solved a problem. Congratulate them briefly, then suggest the next problem to tackle based on their roadmap or weakest topic. Use the get-next-suggestion tool if available.",
    ...
    },
    {
        ...
        system_prompt_modifier: "Explain the concept in depth. Cover the intuition, why it works, common variations, and when to use it. Use analogies and examples.",
    },
    {
        ...
        trigger: "always",
        system_prompt_modifier: "If the user has shared a DSA roadmap or study plan, use it to guide your recommendations. Always be aware of their current position in the roadmap and suggest next steps proactively.",
...
export async function deleteUserSkill(id) {
    const settings = await Storage.getSettings();
    const existing = Array.isArray(settings[SK_USER_SKILLS]) ? settings[SK_USER_SKILLS] : [];
    await Storage.setSettings({ ...settings, [SK_USER_SKILLS]: existing.filter(s => s.id !== id) });
...
export async function getActiveSkills(ctx = {}) {
    ...
    return all.filter(skill => {
        ...
        if (trigger === "on_stuck") {
            const keywords = skill.conditions?.keywords || ["stuck", "hint", "help", "confused", "don't understand"];
            return keywords.some(kw => textLower.includes(kw));
        }

        if (trigger === "on_error") {
            return textLower.includes("error") || textLower.includes("fail") || textLower.includes("wrong answer");
        }
