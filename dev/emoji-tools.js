...
const DEFAULT_ROOTS = ["src", "worker", "docs"];
...
const DEFAULT_SKIP_DIRS = new Set([".git", "node_modules", "dist", "releases", "graphify-out"]);
...
function fixMojibake(text) {
  ...
  while (i < text.length) {
    ...
    if (firstByte >= 0xc2 && firstByte <= 0xf4) {
      const expectedLen = firstByte >= 0xf0 ? 4 : firstByte >= 0xe0 ? 3 : 2;
      ...
      if (seq.length === expectedLen) {
        ...
        if (ok) {
          ...
          if (!decoded.includes("�") && singleCp && decoded.codePointAt(0) > 127) {
            result += decoded;
            i += expectedLen;
            changed = true;
            continue;
...
function runRepair(args) {
  ...
  console.log(`Done. Files ${args.dryRun ? "to repair" : "repaired"}: ${changed}`);
  return 0;
}
...
function runEncode(args) {
  ...
  console.log(`Done. Files ${args.dryRun ? "to encode" : "encoded"}: ${changed}`);
  return 0;
}
...
function runReport(args) {
  ...
  for (let byteOff = 0; byteOff < buffer.length; byteOff++) {
    ...
    const hex = seqBytes.map((x) => x.toString(16).padStart(2, "0")).join("");
    const decoded = Buffer.from(seqBytes).toString("utf8");
    const cp = decoded.codePointAt(0);

    report.push(`@${byteOff} [${hex}] U+${cp.toString(16).padStart(4, "0")} '${decoded}'`);
  ...
  const reportPath = path.join(path.dirname(__dirname), "dev", "emoji-report.txt");
...
Options:
  --roots=src,worker,docs  Override scan/repair roots (default: src,worker,docs).
