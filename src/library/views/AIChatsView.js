...
import {
    ...
    PROMPT_MODE_PREFIXES,
} from "../../ui/components/AIPromptModeSelector.js";
...
export function AIChatsView({
    ...
    useEffect(() => {
        ...
        const prefillProblem = chatSlug
            ? problemIndex.get(chatSlug) || null
            : null;
        setComposeOpen(true);
    ...
    const stats = useMemo(() => {
        ...
        chats.forEach((chat) => {
            ...
            (chat.attachedProblemSlugs || []).forEach((slug) =>
                problemSet.add(slug)
            );
        });
    ...
    const sendNewChat = async () => {
        ...
        try {
            ...
            const context = buildAIChatContext({
                ...
                attachedProblems: selectedAttachments.map((p) => ({
                    slug: p.slug,
                    title: p.title,
                    platform: p.platform,
                    url: p.url,
            ...
            const response = await new Promise((resolve, reject) => {
                if (
                    ...
                    reject(
                        new Error(
                            "AI chat is only available inside the extension."
                        )
                    );
                    return;
                }
                chrome.runtime.sendMessage(
                    ...
                    (resp) => {
                        if (chrome.runtime.lastError)
                            ...
                            reject(
                                new Error(resp?.error || "AI request failed")
                            );
                    }
                );
            });
            ...
            const chatId = await saveAIChat(
                primary?.slug || "library-chat",
                primary?.url || "",
                messages,
                primary?.platform || "library",
                meta
            );
            const allChats = await getAllChats();
            setChats(allChats);
            setSelectedChat(
                allChats.find((chat) => chat.id === chatId) || null
            );
            setComposeText("");
        ...
        try {
            ...
            const nextMeta = {
                problemTitle:
                    ...
                    "AI Study Chat",
                problemTags:
                    selectedChat.problemTags || primaryAttachment?.tags || [],
                attachedProblemSlugs: selectedChat.attachedProblemSlugs || [],
                attachedProblems:
                    selectedChat.attachedProblems ||
                    (primaryAttachment
                ...
                surface: "library-chat",
                requestType:
                    context.requestType || selectedChat.requestType || "",
                usedCommands:
                    context.usedCommands || selectedChat.usedCommands || [],
    ...
    return html`
        <div class="flex flex-col gap-4 w-full h-full min-h-0">
            ...
            ${composeOpen
                ? html`
                      <div
                          class="rounded-2xl border border-cyan-500/20 bg-slate-950/80 p-4 flex flex-col gap-4"
                      >
                          <div class="flex items-center justify-between gap-2">
                              <div>
                                  ...
                                  <p class="text-xs text-slate-500">
                                      Attach one or more problems, then ask for
                                      a solution, explanation, optimization, or
                                      review.
                                  </p>
                              </div>
                              <button
                          ...
                          <div
                              class="grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-0"
                          >
                                      ...
                                      ${visibleProblems.map((problem) => {
                                          ...
                                          return html`
                                              <button
                                                  type="button"
                                                  onClick=${() =>
                                                      toggleAttachment(problem)}
                                                  class="px-3 py-2 rounded-xl border text-left transition-colors ${active
                                                      ? "border-cyan-500/30 bg-cyan-500/10 text-cyan-100"
                                                      : "border-white/10 bg-white/5 text-slate-300 hover:bg-white/10"}"
                                              >
                                                  <div
                                                      class="text-sm font-medium truncate max-w-56"
                                                  >
                                                      ${problem.title}
                                                  </div>
                                                  <div
                              ...
                              <div class="flex flex-col gap-3 min-h-0">
                                  ...
                                  >
                                      ${selectedAttachments.map(
                                          (problem) => html`
                                              <span
                                                  class="px-2 py-1 rounded-full text-[10px] bg-white/5 border border-white/10 text-slate-300"
                                                  >${problem.title} ✕</span
                                              >
                                          `
                                      )}
                                  ...
                                  <div class="flex justify-end gap-2">
                                      <button
                                          onClick=${() => setComposeOpen(false)}
                                          class="px-3 py-2 rounded-lg border border-white/10 text-slate-400 hover:text-slate-200"
                                      >
                                          Cancel
                                      </button>
                                      <button
                                          ...
                                          disabled=${sending ||
                                          !composeText.trim()}
                                          class="px-4 py-2 rounded-lg border border-cyan-500/30 bg-cyan-500/20 text-cyan-100 disabled:opacity-40"
                                      >
                                          Create Chat
                                      </button>
                                  </div>
            ...
            <div class="flex flex-1 gap-4 min-h-0">
                <div
                    class="w-80 min-w-80 bg-slate-900/50 rounded-2xl border border-slate-700 overflow-y-auto"
                >
                    ${activeTab === "by-problem"
                        ? Object.entries(groupedChats.byProblem).map(
                              ([slug, problemChats]) => html`
                                  <div class="border-b border-slate-700">
                                      <div
                                          class="sticky top-0 px-4 py-2 bg-slate-800/90 backdrop-blur font-semibold text-xs text-slate-300 truncate"
                                      >
                                          ${slug} (${problemChats.length})
                                      </div>
                                      ${problemChats.map(
                                          (chat) => html`
                                              <button
                                                  ...
                                                  class="w-full text-left px-4 py-3 border-b border-slate-800 hover:bg-slate-800 transition-colors ${selectedChat?.id ===
                                                  chat.id
                                                      ? "bg-slate-700"
                                                      : ""}"
                                              >
                                                  ...
                                                  ${chat.requestType
                                                      ? html`<div class="mt-2">
                                                            <span
                                                                class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] border border-cyan-500/20 bg-cyan-500/10 text-cyan-200 uppercase tracking-[0.12em]"
                                                                >${chat.requestType}</span
                                                            >
                                                        </div>`
                                                      : ""}
                                                  <div
                        ...
                        : Object.entries(groupedChats.byDate).map(
                              ([dateKey, dateChats]) => html`
                                  <div class="border-b border-slate-700">
                                      <div
                                          class="sticky top-0 px-4 py-2 bg-slate-800/90 backdrop-blur font-semibold text-xs text-slate-300"
                                      >
                                          ${dateKey}
                                      </div>
                                      ${dateChats.map(
                ...
                <div
                    class="flex-1 bg-slate-900/50 rounded-2xl border border-slate-700 p-4 overflow-y-auto min-h-0 flex flex-col"
                >
                    ${selectedChat
                        ? html`
                              <div class="flex flex-col gap-4 h-full">
                                  <div
                                      class="flex items-start justify-between border-b border-slate-700 pb-3 gap-3"
                                  >
                                      <div>
                                          <h3
                                          ...
                                          ${selectedChat.requestType
                                              ? html`<div class="mt-2">
                                                    <span
                                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] border border-cyan-500/20 bg-cyan-500/10 text-cyan-200 uppercase tracking-[0.12em]"
                                                        >${selectedChat.requestType}</span
                                                    >
                                                </div>`
                                              : ""}
                                          ${selectedChat.aiProvider
                                              ? html`<div
                                                    class="text-xs text-cyan-400 mt-1"
                                                >
                                                    🤖
                                      ...
                                      <div class="flex items-center gap-2">
                                          ...
                                          >
                                              ...
                                              >
                                                  ...
                                                  />
                                                  <path
                                                      d="M12 13v8M12 3v6M19 6v6M5 20v-6"
                                                  />
                                              </svg>
                                          </button>
                                          <button
                                              ...
                                              class="px-2 py-1 rounded border border-cyan-500/30 text-cyan-300 text-xs hover:bg-cyan-500/10"
                                          >
                                              Continue
                                          </button>
                                          <button
                                  ...
                                  ${selectedChat.attachedProblems?.length
                                      ? html`
                                            <div class="flex flex-wrap gap-2">
                                                ${selectedChat.attachedProblems.map(
                                                    (problem) => html`
                                                        <span
                                                            class="px-2 py-1 rounded-full text-[10px] bg-white/5 border border-white/10 text-slate-300"
                                                            >${problem.title}</span
                                      ...
                                      ${(selectedChat.messages || []).map(
                                          (msg) => html`
                                              <div class="flex gap-2">
                                                  ...
                                                  <div
                                                      class="flex-1 bg-slate-800 rounded-lg p-3"
                                                  >
                                                      ${msg.role === "user"
                                                          ? html`<div
                                                          ...
                                                          : html`<${AIMarkdownRenderer}
                                                            ...
                                                            />`}
                                                      <div
                                                          class="text-[10px] text-slate-600 mt-2"
                                                      >
                                                          ${formatTime(
                                                              msg.timestamp ||
                                                                  selectedChat.createdAt
                                  ...
                                  >
                                      ...
                                      <${MultiLineAIChatInput}
                                          ...
                                          problem=${selectedChat
                                              .attachedProblems?.[0] || null}
                                          commandItems=${CHAT_COMMANDS}
                                          mentionItems=${AI_MENTION_OPTIONS}
                                      />
                                      <div
                ...
                html`
                    <div
                        class="w-96 bg-slate-900/50 rounded-2xl border border-slate-700 overflow-hidden flex flex-col"
                    >
                        <${MCPToolsSidebar}
