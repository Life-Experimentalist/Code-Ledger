...
function _makeBtn(id, title, svgPath, onClick) {
    ...
    btn.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">${svgPath}</svg>`;
    btn.onmouseenter = () => { btn.style.background = "rgba(255,255,255,0.06)"; btn.style.color = "#e2e8f0"; };
...
function makeAIBtn(onClickFn) {
    return _makeBtn(
        ...
        (e) => { e.preventDefault(); onClickFn?.(); }
    );
}

function findEditorToolbar() {
    const candidates = [
        document.querySelector(".ace_toolbar"),
        document.querySelector('[class*="editor_header"]'),
        document.querySelector('[class*="problems_editor"] [class*="header"]'),
        document.querySelector(".problems-editor-header"),
        document.querySelector('[class^="problems_header_menu"]'),
        (() => {
            ...
            for (let i = 0; i < 6 && el; i++, el = el.parentElement) {
                ...
                if (prev?.querySelector("button, select")) return prev;
            }
            return null;
        })(),
    ].filter(Boolean);
...
export function injectGFGQoL(opts = {}) {
    ...
    if (copyOk && aiOk) { _injected = true; return; }
