...
modalTabRegistry.register("leetcode", [
  ...
  },
  {
    ...
    render(problem, { html, AIMarkdownRenderer }) {
      return html`
        <div class="text-sm text-slate-300 leading-relaxed prose-sm">
          ${problem.aiReview
            ? html`<${AIMarkdownRenderer} content=${problem.aiReview} copyableEnabled=${false} />`
            : html`<p class='text-slate-500'>No AI review available.</p>`}
        </div>`;
    },
  },
  {
