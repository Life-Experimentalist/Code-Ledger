...
export function AnalyticsView({ problems, onNavigate }) {
  ...
  const handleLangClick = useCallback(
    (label) => {
      openDrilldown(
        label,
        (p) => normalizeLang(p.lang?.name || p.language || "") === label,
        "language",
      );
    },
    [openDrilldown],
  ...
  const handlePlatformClick = useCallback(
    (label) => {
      openDrilldown(
        label,
        (p) => (PLATFORM_META[p.platform]?.name || p.platform) === label,
        "platform",
      );
    },
    [openDrilldown],
        ...
        <div class="flex flex-col gap-4">
          ...
          >
            <p class="text-[11px] text-slate-400 mb-1">
              Unsolved from the Blind 75 list, tailored to your gaps:
            </p>
            ${unsolvedNext.length === 0
              ? html`<p class="text-xs text-emerald-400 py-4 text-center">
                          ...
                          <button
                            ...
                            onClick=${() => {
                              ...
                              updateQueryParams({
                                section: null,
                                sectionFilter: null,
                                problem: first?.id || first?.titleSlug,
                              });
                            }}
                          >
                            Browse ←→
