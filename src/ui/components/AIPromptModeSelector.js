...
export const PROMPT_MODES = [
  { id: "tutor",    label: "Tutor",    title: "Guide me toward the solution with hints"         },
  { id: "debug",    label: "Debug",    title: "Help me find and fix errors in my code"           },
  { id: "optimize", label: "Optimize", title: "Suggest performance and style improvements"        },
  { id: "review",   label: "Review",   title: "Provide a full code review with feedback"         },
  { id: "custom",   label: "Custom",   title: "No system prompt — free-form conversation"        },
];
...
export function AIPromptModeSelector({ mode = "tutor", onChange }) {
  return html`
    <div class="flex items-center gap-1 flex-wrap">
      ${PROMPT_MODES.map(({ id, label, title }) => html`
        <button
          key=${id}
          title=${title}
          onClick=${() => onChange?.(id)}
