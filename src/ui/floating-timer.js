...
function loadState(slug) {
    try {
        return JSON.parse(sessionStorage.getItem(SESSION_KEY(slug)) || "null");
    } catch {
        return null;
    }
}
function saveState(slug, st) {
    try {
        sessionStorage.setItem(SESSION_KEY(slug), JSON.stringify(st));
    } catch {}
}
...
export function createFloatingTimer(slug = "", opts = {}) {
    ...
    Object.assign(root.style, {
        ...
        color: "#e2e8f0",
        boxShadow:
            "0 4px 24px rgba(0,0,0,0.5), 0 0 0 1px color-mix(in srgb, var(--cl-accent, #06b6d4) 10%, transparent)",
        userSelect: "none",
        cursor: "grab",
    ...
    display.style.cssText =
        "min-width:46px;font-variant-numeric:tabular-nums;letter-spacing:0.05em;color:var(--cl-accent, #06b6d4);font-weight:600;";
    ...
    root.addEventListener("mousedown", (e) => {
        if (e.target !== root && e.target !== icon && e.target !== display)
            return;
        dragging = true;
