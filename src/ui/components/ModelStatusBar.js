...
export function ModelStatusBar({ settings }) {
  ...
  return html`
    <div class="flex items-center gap-2 px-2 py-1 rounded-md bg-slate-800/60 border border-slate-700/40 text-[11px]" title="Active AI model">
      ...
      ${fallbackProvider && html`
        ...
        ${fallbackModel && html`<span class="text-slate-600 text-[10px]">${shortModel(fallbackModel)}</span>`}
      `}
    </div>
  `;
}
