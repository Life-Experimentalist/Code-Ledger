...
function extractRequestPayload(issue) {
  ...
  const blockMatch = body.match(
    /<!--\s*codeledger-canonical-request\s*-->[\s\S]*?```json\s*([\s\S]*?)\s*```/i,
  );
  if (blockMatch?.[1]) {
    try {
  ...
  return {
    canonicalId: slugify(
      titleMatch?.[1] || issue.title.replace(/^\[Canonical\]\s*/i, ""),
    ),
...
async function countValidUpvotes(issue) {
  ...
  for (const reaction of reactions) {
    ...
    if (user.created_at && new Date(user.created_at).getTime() > issueCreatedAt)
      continue;
    count += 1;
  }
...
async function validateWithGemini(issue, payload, voteCount) {
  ...
  if (!keys.length) {
    throw new Error(
      "No Gemini API keys configured. Set GEMINI_API_KEYS or GEMINI_API_KEY_1..N.",
    );
  }
  ...
  throw (
    lastErr || new Error("Gemini validation failed with all available keys.")
  );
}

function buildCanonicalEntry(issue, payload, voteCount, validation) {
  const aliasesFromPayload = Array.isArray(payload.aliases)
    ? payload.aliases
    : [];
  const aliasesFromValidation = Array.isArray(validation.aliases)
    ? validation.aliases
    : [];
  const aliases = [...aliasesFromPayload, ...aliasesFromValidation]
    ...
    .filter(
      (alias, index, arr) =>
        ...
        arr.findIndex(
          (item) =>
            item.platform === alias.platform && item.slug === alias.slug,
        ),
    );
  ...
  if (!/^(easy|medium|hard)$/i.test(difficulty)) {
    throw new Error(
      "Gemini returned an invalid difficulty. Expected Easy, Medium, or Hard.",
    );
  }
  ...
  return {
    ...
    tags: Array.isArray(validation.tags)
      ? validation.tags.filter(Boolean).map(String)
      : [],
    aliases,
    voteCount: Number.isFinite(validation.voteCount)
...
async function loadCanonicalMap() {
  ...
  if (!parsed || typeof parsed !== "object") {
    throw new Error(
      "canonical-map.json must be an object with an entries array",
    );
  }
...
function hasCanonicalConflict(entries, candidate) {
  return entries.some((entry) => {
    if (entry.canonicalId && entry.canonicalId === candidate.canonicalId)
      return true;
    if (
      String(entry.canonicalTitle || "").toLowerCase() ===
      String(candidate.canonicalTitle || "").toLowerCase()
    )
      return true;
    ...
    return candidate.aliases.some((alias) =>
      existingAliases.some(
        (existing) =>
          existing.platform === alias.platform && existing.slug === alias.slug,
      ),
    );
  });
}
...
async function commitCanonicalMap(updated) {
      ...
      execFile(
        ...
        (err, stdout, stderr) => {
          if (err) {
            reject(
              new Error(
                `${cmd} ${args.join(" ")} failed: ${stderr || stdout || err.message}`,
              ),
            );
            return;
          }
  ...
  try {
    await run("git", [
      "commit",
      "-m",
      "feat: merge canonical mapping from GitHub issues",
    ]);
  } catch (err) {
  ...
  await run("git", [
    "push",
    "origin",
    `HEAD:${process.env.GITHUB_REF_NAME || "main"}`,
  ]);
  return true;
}
...
async function processIssue(issue) {
  ...
  if (voteCount < votesRequired) {
    return {
      status: "skipped",
      reason: `Waiting for ${votesRequired - voteCount} more valid upvote(s).`,
    };
  }
  ...
  if (String(validation.decision || "").toLowerCase() !== "yes") {
    return {
      status: "rejected",
      reason: validation.reason || "Gemini returned no.",
    };
  }
  ...
  if (hasCanonicalConflict(existingEntries, candidate)) {
    await comment(
      issue.number,
      `Canonical mapping already exists for **${candidate.canonicalTitle}**. The issue will be closed.`,
    );
...
async function main() {
  ...
  for (const issue of issues) {
    // eslint-disable-next-line no-await-in-loop
    const result = await processIssue(issue).catch((err) => ({
      status: "error",
      reason: err.message,
    }));
    results.push({ issue: issue.number, ...result });
    console.log(
      `Issue #${issue.number}: ${result.status}${result.reason ? ` - ${result.reason}` : ""}`,
    );
  }

  const merged = results.filter((item) => item.status === "merged").length;
  const alreadyExists = results.filter(
    (item) => item.status === "already-exists",
  ).length;
  ...
  console.log(
    JSON.stringify(
      { merged, alreadyExists, rejected, skipped, errors },
      null,
      2,
    ),
  );
