    ...
    async (provider) => {
      ...
      try {
        ...
        if (count === 0) {
          setRepoSyncStatus((s) => ({
            ...s,
            [provider]: "Already in sync — no missing problems found.",
          }));
          return;
        }
                ...
                ${(() => {
                  ...
                  return html`
                    <button
                      onClick=${() =>
                        isEndpoint
                          ? handleTestEndpoint(prov, values[f.key] ?? "", f.key)
                          : handleTestKey(
                              prov,
            ...
            <${ConflictResolutionModal}
              ...
              onCancel=${async (resolvedSoFar, remaining) => {
                ...
                if (resolvedSoFar?.length > 0) {
                  try {
                    await applySyncImport(resolvedSoFar);
                  } catch (_) {}
                }
                // Re-queue unresolved conflicts so the banner reappears
                if (remaining?.length > 0) {
                  setSyncConflictData((prev) => ({
                    ...prev,
                    conflicts: remaining,
                  }));
                } else {
