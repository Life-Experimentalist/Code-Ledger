...
export function AnalyticsView({ problems, onNavigate }) {
    ...
    const stats = useMemo(() => {
        ...
        for (let i = 0; i < 730; i++) {
            ...
            if (dayMap[dateStr(d)]) {
                ...
                if (i === 0)
                    streak = 0; // today no solve — streak = 0
                else if (streak > 0) {
        ...
        if (timed.length > 0) {
            s.avgSolveSeconds = Math.round(
                timed.reduce((acc, p) => acc + p.elapsedSeconds, 0) /
                    timed.length
            );
        }
    ...
    const openDrilldown = useCallback(
        (label, filterFn, sectionType) => {
            setDrilldown({
                label,
                problems: problems.filter(filterFn),
                sectionType,
            });
            if (sectionType)
                updateQueryParams({
    ...
    const handleLangClick = useCallback(
        (label) => {
            openDrilldown(
                label,
                (p) =>
                    normalizeLang(p.lang?.name || p.language || "") === label,
                "language"
            );
        },
        [openDrilldown]
    ...
    const handlePlatformClick = useCallback(
        (label) => {
            openDrilldown(
                label,
                (p) =>
                    (PLATFORM_META[p.platform]?.name || p.platform) === label,
                "platform"
            );
        },
        [openDrilldown]
    ...
    useEffect(() => {
        ...
        if (section === "difficulty") {
            ...
            openDrilldown(
                sectionFilter,
                (p) =>
                    normalizeLang(p.lang?.name || p.language || "") ===
                    sectionFilter
            );
            ...
            openDrilldown(
                sectionFilter,
                (p) =>
                    (p.tags || []).includes(sectionFilter) ||
                    p.topic === sectionFilter
            );
        }
        ...
        return {
            ...
            platformBar: {
                ...
                datasets: [
                    {
                        label: "Problems",
                        data: Object.values(stats.platforms).map(
                            (p) => p.total
                        ),
    ...
    return html`
        <div class="flex flex-col gap-6 w-full pb-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ${[
                    ...
                    stats.avgSolveSeconds > 0
                        ? (() => {
                              const h = Math.floor(
                                  stats.avgSolveSeconds / 3600
                              );
                              const m = Math.floor(
                                  (stats.avgSolveSeconds % 3600) / 60
                              );
                              ...
                              return {
                                  ...
                                  value: val,
                                  sub: "avg across timed problems",
                                  color: "#ec4899",
                              };
                          })()
                        : {
                              label: "Languages",
                              value: Object.keys(stats.langs).length,
                              sub:
                    ...
                    (card) => html`
                        <div
                            class="p-4 bg-[#0a0a0f] border border-white/5 rounded-2xl flex flex-col gap-1 relative overflow-hidden"
                        >
                            <div
                                class="absolute inset-0 opacity-5"
                                style=${{
                                    background: `radial-gradient(circle at 0% 0%, ${card.color}, transparent 60%)`,
                                }}
                            ></div>
                            <span
            ...
            ${Object.keys(stats.platforms).length > 0
                ? html`
                      <div
                          class="grid gap-4"
                          style=${{
                              gridTemplateColumns: `repeat(${Math.min(3, Object.keys(stats.platforms).length)}, 1fr)`,
                          }}
                      >
                                      ...
                                      >
                                          ...
                                          <div class="flex gap-3 text-[11px]">
                                              <span class="text-emerald-400"
                                                  >${counts.easy}E
                                                  (${pct(counts.easy)}%)</span
                                              >
                                              <span class="text-amber-400"
                                                  >${counts.medium}M
                                                  (${pct(counts.medium)}%)</span
                                              >
                                              <span class="text-rose-400"
                                                  >${counts.hard}H
                                                  (${pct(counts.hard)}%)</span
                                              >
                                          </div>
                                          <div
                                          ...
                                          >
                                              <div
                                                  ...
                                                  style=${{
                                                      width: `${pct(counts.easy)}%`,
                                                  }}
                                              ></div>
                                              <div
                                                  class="h-full bg-amber-500"
                                                  style=${{
                                                      width: `${pct(counts.medium)}%`,
                                                  }}
                                              ></div>
                                              <div
                                                  class="h-full bg-rose-500"
                                                  style=${{
                                                      width: `${pct(counts.hard)}%`,
                                                  }}
                                              ></div>
                                          </div>
                                      </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ${topTopics.map(([topic, counts]) => {
                            const barPct = Math.round(
                                (counts.total / maxTopicCount) * 100
                            );
                            return html`
                                <div
                                ...
                                >
                                    ...
                                    <div
                                        class="flex gap-3 mt-1.5 text-[10px] text-slate-600"
                                    >
                                        ${counts.easy
                                            ? html`<span
                ...
                <div class="flex flex-col gap-4">
                    ...
                    >
                        <p class="text-[11px] text-slate-400 mb-1">
                            Unsolved from the Blind 75 list, tailored to your
                            gaps:
                        </p>
                        ${unsolvedNext.length === 0
                            ? html`<p
                                  class="text-xs text-emerald-400 py-4 text-center"
                              >
                                  You've completed the Blind 75!
                              </p>`
                            : unsolvedNext.map(
                                  (rec) => html`
                                      <a
            ...
            ${drilldown
                ? html`
                      <div
                          class="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4"
                          onClick=${(e) => {
                      ...
                      >
                          <div
                              class="bg-[#0d0d14] border border-white/10 rounded-2xl w-full max-w-lg max-h-[70vh] flex flex-col shadow-2xl"
                          >
                              <div
                              ...
                              >
                                  <div class="flex flex-col">
                                      ...
                                      <span class="text-[11px] text-slate-500"
                                          ...
                                          problem${drilldown.problems.length !==
                                          1
                                              ? "s"
                                              : ""}</span
                                      >
                                  ...
                                  <div class="flex items-center gap-2">
                                      ...
                                      <button
                                          ...
                                          class="text-slate-500 hover:text-white text-xl leading-none px-1"
                                      >
                                          ✕
                                      </button>
                                  </div>
                                                ...
                                                return html`
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                            <span
                                                                class="text-[11px] ${diffCls} shrink-0"
                                                                >${p.difficulty ||
                                                                "?"}</span
                                                            >
                                                        </div>
            ...
            <${ProblemModal}
                ...
                problemList=${modalProblemList.length
                    ? modalProblemList
                    : problems}
