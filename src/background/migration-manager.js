...
export async function migrateProblemIds() {
    ...
    const toMigrate = problems.filter(
        (p) => !ALREADY_MIGRATED.test(p.id || "")
    );
...
export async function migrateRepo() {
    ...
    dbg.log(
        `migrateRepo(): prepared ${newFiles.length} file(s) for new layout`
    );

    if (newFiles.length === 0 && oldPaths.length === 0) {
        dbg.log(
            "migrateRepo(): ✓ nothing to migrate (repo already in new layout)"
        );
...
export async function resetRepo() {
    ...
    const filesToCommit = [...desiredFiles.entries()].map(
