...
const DEFAULT_REPO_TOPICS = [
    "codeledger", "dsa", "leetcode", "algorithms",
    "dynamic-programming", "graphs", "trees", "arrays", "hashing", "greedy",
];
...
export function GitHubOnboardingModal({ isOpen, onComplete, username, token }) {
    ...
    const ghFetch = async (path, opts = {}) => {
        ...
        if (!res.ok) {
            ...
            throw Object.assign(
                new Error(e.message || `GitHub API ${res.status}`),
                { status: res.status, body: e }
            );
        }
        return res.json();
    };
    ...
    const scheduleNameCheck = (name, owner) => {
        ...
        if (!clean || !owner) { setNameCheck(null); return; }
    ...
    const namePreview = cleanPreview && cleanPreview !== repoName
        ? html`<p class="text-[10px] text-slate-400 mt-1">Will be created as: <span class="text-cyan-400 font-mono">${cleanPreview}</span></p>`
        : "";
    ...
    const nameCheckBadge =
        nameCheck === "checking"
            ...
            : nameCheck === "taken"
            ? html`<span class="text-[10px] text-rose-400">✗ Already exists under ${selectedOwner}</span>`
            : "";

    // ── Step labels ───────────────────────────────────────────────────────────

    const stepLabel = { check: "", already: "", choice: "Step 1 of 2", new: "Step 2 of 2", existing: "Step 2 of 2", done: "Setup complete" }[step] || "";
    const stepTitle = { check: "Loading…", already: "Repository Connected", choice: "Set Up GitHub", new: "Create Repository", existing: "Link Existing Repository", done: "All Set! 🎉" }[step] || "";
    ...
    return html`
        ...
        <div class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div class="bg-[#0a0a0f] border border-cyan-500/20 rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
                              ...
                              <div class="space-y-5 text-center">
                                  ...
                                  <div>
                                      ...
                                      <div class="mt-3 p-3 rounded-xl bg-white/5 border border-white/8 text-left">
                                          <p class="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Connected repository</p>
                    ...
                    ${step === "new"
                        ? html`
                              <div class="space-y-4">
                                  ...
                                  <div>
                                      ...
                                      <input
                                          ...
                                          class="w-full px-3 py-2 bg-black border ${
                                              nameCheck === "taken" ? "border-rose-500/50" :
                                              nameCheck === "available" ? "border-emerald-500/50" :
                                              "border-white/10"
                                          } rounded-lg text-white text-sm placeholder-slate-600 focus:outline-none disabled:opacity-50"
                                  ...
                                  ${error
                                      ? html`<div class="p-3 rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs whitespace-pre-wrap">${error}</div>`
                                      : ""}
                                  ${progress && !error
                                      ? html`
                                  ...
                                  <div class="flex gap-3 pt-2">
                                      <button
                                          onClick=${() => { setStep("choice"); setError(""); setNameCheck(null); }}
                                          disabled=${busy}
                                      ...
                                      <button
                                          ...
                                          disabled=${busy || !sanitize(repoName) || nameCheck === "taken" || nameCheck === "checking"}
                                  ...
                                  ${reposLoading
                                      ...
                                      : html`
                                            <div>
                                                ...
                                                ${userRepos.length === 0
                                                    ? html`<p class="text-xs text-slate-500">No repositories found for <span class="text-slate-300">${selectedOwner}</span>.</p>`
                                                    : html`
                                                          <select
                                                              value=${selectedRepo}
                                                          ...
                                                          ${selectedRepo
                                                              ? html`<p class="text-[10px] text-slate-500 mt-1 font-mono">${selectedOwner}/${selectedRepo}</p>`
                                                              : ""}
                                                      `}
                                            </div>
                                        `}
                        ...
                        ? html`
                              <div class="space-y-4 text-center">
                                  ...
                                  <div class="p-4 rounded-xl bg-cyan-500/5 border border-cyan-500/20">
                                      ...
                                      <p class="text-sm font-mono text-cyan-300">${finalOwner || username}/${finalRepo}</p>
                                      <a
                                  ...
                                  ${postSyncState?.phase === "syncing"
                                      ...
                                      ? html`
                                        ...
                                        `
                                      : postSyncState === "error"
                                      ? html`<p class="text-xs text-amber-400">Could not auto-sync — use the Sync button in Git settings.</p>`
                                      : ""}
                                  <button
                                      onClick=${onComplete}
