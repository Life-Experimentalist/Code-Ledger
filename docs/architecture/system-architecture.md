...
async function getQueueStats() {
    ...
    dbg.log(`getQueueStats(): ✓ pending=${stats.pending}, processing=${stats.processing}, done=${stats.done}, failed=${stats.failed}`);
    return stats;
}
```
...
async function importFromRepo() {
    ...
    const remoteIndex = await ghGetContents(
        owner, repo, "index.json"
    );
