...
export function initializeHandlers() {
  ...
  platforms.forEach(h => {
    ...
    if (typeof h.getSettingsSchema === 'function') registry.registerSettings(h.id, h.getSettingsSchema());
  });
  ...
  const ais = [new GeminiHandler(), new OpenAIHandler(), new ClaudeHandler(), new DeepSeekHandler(), new OllamaHandler()];
  ais.forEach(h => {
