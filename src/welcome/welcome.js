...
const STEPS = [
  { id: "installed", icon: "🧩", label: "Extension installed",    desc: "CodeLedger is running in your browser." },
  { id: "github",    icon: "🔗", label: "GitHub connected",       desc: "Authorized with GitHub so commits can be made on your behalf." },
  { id: "repo",      icon: "📁", label: "Repository linked",      desc: "A GitHub repo is configured to receive your solutions." },
  { id: "solve",     icon: "✅", label: "First problem solved",    desc: "Solve any accepted problem on LeetCode, GeeksForGeeks, or Codeforces." },
  { id: "commit",    icon: "💾", label: "First commit to GitHub",  desc: "Your solution was automatically committed to your repository." },
];
...
function sw(type, extra = {}) {
  return new Promise((res, rej) => {
    if (!chrome?.runtime?.id) { rej(new Error("No extension runtime")); return; }
...
function DiagnosticsPanel() {
  ...
  const flash = (text, isErr = false) => { setMsg({ text, isErr }); setTimeout(() => setMsg(""), 5000); };
  ...
  const runRepair = async (action, label) => {
    if (!confirm(`Run "${label}"? This will commit changes to your GitHub repo.`)) return;
    setRepairing(action);
    try {
  ...
  const runMigration = async () => {
    setMigrating(true);
    try {
      const r = await sw("CODELEDGER_RUN_MIGRATIONS", { mode: migrationMode });
  ...
  const StatusBadge = ({ status }) => {
    const map = { ok: ["✓", "text-emerald-400 bg-emerald-500/10"], missing: ["✗ missing", "text-rose-400 bg-rose-500/10"], error: ["? error", "text-amber-400 bg-amber-500/10"] };
    const [label, cls] = map[status] || ["?", "text-slate-400 bg-white/5"];
    return html`<span class="px-2 py-0.5 rounded text-xs font-mono ${cls}">${label}</span>`;
  };

  const overallHealthy = checks && checks.layoutUpToDate && checks.infraOk && !checks.hasOldLayout && (checks.uncommittedCount <= 0);

  return html`
    <div class="flex flex-col gap-5">
      <div class="flex items-center justify-between">
        <p class="text-sm text-slate-400">Scan your GitHub repo for issues and apply fixes.</p>
        <button
      ...
      ${msg && html`<p class="text-xs px-3 py-2 rounded-lg border ${msg.isErr ? "border-rose-500/30 bg-rose-500/5 text-rose-300" : "border-emerald-500/30 bg-emerald-500/5 text-emerald-300"}">${msg.text}</p>`}

      ${!checks && !loading && html`
        <div class="p-6 bg-white/3 border border-white/8 rounded-xl text-center">
          <p class="text-sm text-slate-500">Click <strong class="text-slate-300">Run Scan</strong> to check your repository health.</p>
        </div>
      `}
      ...
      ${checks && html`
        ...
        <div class="p-4 rounded-xl border ${overallHealthy ? "border-emerald-500/30 bg-emerald-500/5" : "border-amber-500/30 bg-amber-500/5"}">
          <div class="flex items-center gap-2">
            <span class="text-lg">${overallHealthy ? "✅" : "⚠️"}</span>
            <div>
              <p class="text-sm font-medium ${overallHealthy ? "text-emerald-300" : "text-amber-300"}">${overallHealthy ? "Repository is healthy" : "Issues found — see below"}</p>
              <p class="text-xs text-slate-500">${checks.owner}/${checks.repo}</p>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-3">
          <div class="p-3 bg-white/5 border border-white/10 rounded-xl">
            <p class="text-xs text-slate-500 mb-1">Local problems</p>
            <p class="text-lg font-semibold text-white">${checks.localProblemCount}</p>
          </div>
          <div class="p-3 bg-white/5 border border-white/10 rounded-xl">
            <p class="text-xs text-slate-500 mb-1">In GitHub repo</p>
            <p class="text-lg font-semibold ${checks.committedProblemCount === null ? 'text-slate-500' : 'text-white'}">${checks.committedProblemCount ?? "—"}</p>
          </div>
          <div class="p-3 bg-white/5 border border-white/10 rounded-xl">
            <p class="text-xs text-slate-500 mb-1">Not committed</p>
            <p class="text-lg font-semibold ${(checks.uncommittedCount || 0) > 0 ? 'text-amber-400' : 'text-emerald-400'}">${Math.max(0, checks.uncommittedCount ?? 0)}</p>
          </div>
        </div>
        ...
        <div class="p-4 bg-white/3 border border-white/8 rounded-xl">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-xs font-medium text-slate-300 uppercase tracking-widest">Repo Layout</h4>
            ${!checks.layoutUpToDate && html`
              <button
                onClick=${() => runRepair("migrate-layout", "Migrate layout")}
                disabled=${!!repairing}
              ...
              >${repairing === "migrate-layout" ? "Migrating…" : "Migrate to v${checks.currentLayoutVersion}"}</button>
            `}
          </div>
          <div class="grid grid-cols-2 gap-3 text-xs">
            <div>
              <span class="text-slate-500">Current</span>
              <span class="ml-2 text-white">v${checks.currentLayoutVersion}</span>
            </div>
            <div>
              <span class="text-slate-500">Repo</span>
              <span class="ml-2 ${checks.layoutUpToDate ? 'text-emerald-400' : 'text-amber-400'}">
                ${checks.repoLayoutVersion !== null ? `v${checks.repoLayoutVersion}` : "unknown"}
                ${checks.layoutUpToDate ? " ✓" : " (outdated)"}
              </span>
            </div>
            ${checks.hasOldLayout && html`
              <div class="col-span-2 mt-1 text-amber-300">
                Old-layout directories detected (${[checks.hasOldTopicsDir && "topics/", checks.hasOldProblemsDir && "problems/"].filter(Boolean).join(", ")}).
        ...
        <div class="p-4 bg-white/3 border border-white/8 rounded-xl">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-xs font-medium text-slate-300 uppercase tracking-widest">Infrastructure Files</h4>
            ${!checks.infraOk && html`
              <button
                onClick=${() => runRepair("rebuild-infra", "Rebuild infra")}
                disabled=${!!repairing}
              ...
              >${repairing === "rebuild-infra" ? "Rebuilding…" : "Rebuild"}</button>
            `}
          </div>
          <div class="flex flex-col gap-2">
            ${Object.entries(checks.infraStatus || {}).map(([path, status]) => html`
              <div key=${path} class="flex items-center justify-between">
                <span class="text-xs text-slate-400 font-mono">${INFRA_LABELS[path] || path}</span>
        ...
        <!-- Migration section -->
        ${((checks.uncommittedCount || 0) > 0) && html`
          <div class="p-4 bg-white/3 border border-white/8 rounded-xl">
            <h4 class="text-xs font-medium text-slate-300 uppercase tracking-widest mb-3">Push Missing Problems</h4>
            <p class="text-xs text-slate-500 mb-3">${checks.uncommittedCount} local problem(s) not yet in GitHub. Choose how to commit them:</p>
            <div class="flex flex-col gap-2 mb-4">
              <label class="flex items-center gap-3 cursor-pointer">
                <input type="radio" name="migMode" value="bulk" checked=${migrationMode === "bulk"} onChange=${() => setMigrationMode("bulk")} />
                <div>
                  <p class="text-xs text-slate-200 font-medium">Bulk commit (recommended)</p>
                  <p class="text-xs text-slate-500">Single atomic commit for all missing problems</p>
                </div>
              </label>
              <label class="flex items-center gap-3 cursor-pointer">
                <input type="radio" name="migMode" value="individual" checked=${migrationMode === "individual"} onChange=${() => setMigrationMode("individual")} />
                <div>
                  <p class="text-xs text-slate-200 font-medium">Individual commits</p>
                  <p class="text-xs text-slate-500">One commit per problem with correct timestamps</p>
                </div>
              </label>
            </div>
            <button
            ...
            >${migrating ? "Pushing…" : `Push ${checks.uncommittedCount} problem(s) →`}</button>
          </div>
        `}

        <!-- Advanced repair -->
        <details class="group">
          <summary class="text-xs text-slate-500 cursor-pointer hover:text-slate-300 transition-colors list-none flex items-center gap-1">
            <span class="group-open:rotate-90 transition-transform inline-block">▶</span>
            Advanced repairs
          </summary>
          <div class="mt-3 flex flex-wrap gap-2">
            <button
              onClick=${() => runRepair("rebuild-infra", "Rebuild infra files")}
              disabled=${!!repairing}
            ...
            >Rebuild infra files</button>
            <button
              onClick=${() => runRepair("migrate-layout", "Migrate repo layout")}
              disabled=${!!repairing}
              class="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 text-xs rounded-lg transition-colors disabled:opacity-50"
            >Re-run layout migration</button>
          </div>
        </details>
      `}
    </div>
...
function WelcomeApp() {
  ...
  const openLibrary  = () => { if (chrome?.runtime?.id) chrome.tabs.create({ url: chrome.runtime.getURL("library/library.html") }); };
  const openSettings = () => { if (chrome?.runtime?.id) chrome.tabs.create({ url: chrome.runtime.getURL("library/library.html") + "?tab=settings&settingsTab=git" }); };
        ...
        <div class="flex gap-1 p-1 bg-white/5 border border-white/10 rounded-xl">
          ${TABS.map(t => html`
            <button
              ...
              class="flex-1 py-2 rounded-lg text-sm transition-colors ${activeTab === t.id
        ...
        <div class="w-full max-w-lg">
          ...
          <div class="flex flex-col gap-3 mb-10">
            ${STEPS.map((step) => {
              ...
              return html`
                <div class="flex items-start gap-4 p-4 rounded-xl border transition-colors ${done ? "border-emerald-500/20 bg-emerald-500/5" : "border-white/5 bg-white/[0.02]"}">
          ...
          <!-- All done -->
          ${allDone && html`
            <div class="mb-8 p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 text-center">
