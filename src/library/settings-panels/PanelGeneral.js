  ...
  return html`
    <div class="space-y-6 w-full">
      ...
      <div class="flex items-center gap-3 pt-2">
        ...
        ${saveMsg &&
        html`
          <span class="text-xs ${saveMsg.includes("failed") ? "text-rose-400" : "text-emerald-400"}"
            >${saveMsg}</span
          >
        `}
      </div>
      ...
      <div class="p-4 rounded-xl border border-white/8 bg-white/2 space-y-4">
        <h3 class="text-xs font-medium text-slate-400 uppercase tracking-widest">Behaviour</h3>
        <div class="flex items-start gap-3">
          <button
            onClick=${() => onSettingsChange?.("remember_modal_tab", !settings?.remember_modal_tab)}
          ...
          >
            <span
              class="inline-block h-3.5 w-3.5 rounded-full bg-white shadow transform transition-transform
                            ${settings?.remember_modal_tab ? "translate-x-4" : "translate-x-0.5"}"
            >
            </span>
          </button>
          <div>
            ...
            <p class="text-[11px] text-slate-500 leading-snug">
              When navigating between problems, reopen the same tab (Overview, Code, Review, etc.)
              if it exists. Falls back to Overview when the tab is unavailable.
            </p>
          </div>
        </div>
      </div>
