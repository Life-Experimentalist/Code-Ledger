...
export function ProblemModal({
    ...
    const handleRefreshData = useCallback(async () => {
        ...
        try {
            ...
            if (!fetched) {
                setChatError(
                    "Refresh timeout. Check the opened tab and try again."
                );
                ...
                chrome.runtime.sendMessage(
                    ...
                    (resp) => {
                        ...
                        if (chrome.runtime.lastError) {
                            ...
                            dbg.warn(
                                "REGENERATE_AI_REVIEW failed:",
                                resp?.error
                            );
                            reject(
                                new Error(resp?.error || "AI review failed")
                            );
                        }
                    }
                );
            ...
            >
                ...
                >
                    ...
                    <div class="flex-1 min-w-0">
                        <h2
                            class="text-base font-semibold text-white leading-snug"
                        >
                            ${problem.title}
                        </h2>
                ...
                ${topics.length
                    ? html`
                          <div
                              class="flex flex-wrap gap-1.5 px-5 pt-3 shrink-0"
                          >
                              ${topics.map(
                                  (t) => html`
                                      <span
                                  ...
                                  ? html`
                                        <div class="flex flex-col gap-0.5">
                                            ...
                                            >
                                                ...
                                                ${problem.runtimePct
                                                    ? html`<span
                                                          class="text-cyan-500/60 text-[10px]"
                                                      >
                                                          · beats
                                                          ${problem.runtimePct.toFixed(
                                                              0
                                                          )}%</span
                                                      >`
                                                    : ""}
                ...
                ${siblings.length
                    ? html`
                          <div
                              class="flex items-center gap-2 px-5 pt-3 shrink-0 flex-wrap"
                          >
                              <span
                              ...
                              ${siblings.map((sib) => {
                                  ...
                                  const sibLang =
                                      sib.lang?.name || sib.language || "?";
                                  const isSamePlatform =
                                      sib.platform === problem.platform;
                                  return html`
                                      <button
                                          onClick=${() =>
                                              onNavigateProblem?.(sib)}
                                      ...
                                      >
                                          ...
                                          ${!isSamePlatform
                                              ...
                                              : ""}
                                          <span
                                              class="font-mono text-cyan-400/80"
                                              >${sibLang}</span
                                          >
                                      </button>
                                  `;
                ...
                ${tabs.length > 1
                    ? html`
                      ...
                      `
                    : html`<div
                          class="border-b border-white/5 shrink-0"
                      ></div>`}
                        ...
                        ? html`
                              <div class="flex flex-col gap-4 h-full">
                                  ${methods.map(
                                      (method, idx) => html`
                                          <div
                                              class="p-3 rounded-lg border ${selectedMethodIdx ===
                                              idx
                                          ...
                                          >
                                              ...
                                              >
                                                  ...
                                                  ${method.description
                                                      ? html`<p
                                                            class="text-[10px] text-slate-400 mb-2"
                                                        >
                                                            ${method.description}
                                                        </p>`
                                                      : ""}
                          ...
                          ? html`
                                <div class="flex flex-col gap-3 h-full">
                                    ...
                                    <div class="text-[10px] text-slate-600">
                                        Notes are saved automatically to your
                                        problem record.
                                    </div>
                                </div>
                            `
...
function EditTab({ problem, onUpdate, onDelete, onClose }) {
    const initialTags = (() => {
        ...
        const fallbackTopic =
            problem.topic && problem.topic !== "Untagged"
                ? [problem.topic]
                : [];
        return [
            ...new Set(
    ...
    const save = async () => {
        ...
        try {
            const newTags = [
                ...new Set(
                    tagList.map((t) => String(t || "").trim()).filter(Boolean)
                ),
            ];
            const updated = {
                ...problem,
        ...
        <div class="flex flex-col gap-1.5">
            ...
            >
                ${tagList.length
                    ...
                    : html`<span class="text-[11px] text-slate-600"
                          >No tags yet</span
                      >`}
            </div>
            ...
            <p class="text-[10px] text-slate-600">
                First tag becomes the primary topic used in analytics and the
                graph.
            </p>
        </div>
