...
export function AICommandPalette({
  ...
  return html`
    <div
      class="rounded-xl border border-slate-700 bg-slate-950/98 shadow-2xl overflow-hidden"
          ...
          : items.map(
              (item, index) => html`
                <button
                  ...
                  class="w-full text-left px-3 py-2 border-b border-slate-900/70 last:border-b-0 transition-colors ${index ===
                  activeIndex
                ...
                >
                  <div class="flex items-start justify-between gap-2">
                    ...
                    <span
                      class="shrink-0 text-[10px] px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-slate-400"
                      >${item.usage || item.kind || ""}</span
                    >
                  </div>
                </button>
              `,
