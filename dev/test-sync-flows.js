    ...
    try {
        ...
        MOCK_LOCAL.forEach(local => {
            ...
            if (!remoteIds.has(key)) {
                ...
                const remote = MOCK_REMOTE.find(r => r.id === local.id && r.lang.name === local.lang.name);
        ...
        const preview = { conflicts, remoteOnly, pendingConflicts: conflicts.length, pendingRemoteOnly: remoteOnly.length };
...
async function testApplyImport(problems) {
    ...
    try {
        ...
        const result = { saved: problems.length, applied: problems.map(p => p.id) };
...
async function testSyncAllCommit() {
    ...
    try {
        ...
        files.push({ path: "index.json", content: JSON.stringify({ problems: MOCK_LOCAL, total: MOCK_LOCAL.length }, null, 2) });

        vlog("Commit payload:", files.length, "files");
        vlog("Commit message: chore: sync", MOCK_LOCAL.length, "problem(s)");

        const commitSha = DRY_RUN ? "abcd1234" : Buffer.from(JSON.stringify(files)).toString("hex").slice(0, 8);

        success("Sync all commit (bulk mode):", files.length, "files →", commitSha);

        return { committed: MOCK_LOCAL.length, filesCount: files.length, commitSha };
    } catch (e) {
...
async function testPrimaryFailureFailover() {
    ...
    try {
        const targets = [
            { provider: "github", owner: "VKrishna04", repo: "solutions", isPrimary: true },
            { provider: "github", owner: "Life-Experimentalist", repo: "solutions-mirror", isPrimary: false },
            { provider: "gitlab", owner: "vkrishna04", repo: "solutions", isPrimary: false },
        ];
        ...
        success("Failover successful: switched to mirror", activeTarget.owner + "/" + activeTarget.repo);
